"use strict";
/**
 * stocobs.ts
 * @author Sidharth Mishra
 * @description Handler for STOCOBS © Alexa skill.
 * @created Mon Jan 22 2018 14:11:26 GMT-0800 (PST)
 * @copyright 2017 Sidharth Mishra
 * @last-modified Fri Jan 26 2018 14:41:10 GMT-0800 (PST)
 */
Object.defineProperty(exports, "__esModule", { value: true });
// =========================================================================================
//                                      IMPORTS
// =========================================================================================
var buffer_1 = require("buffer");
var https_1 = require("https");
/**
 * The `outputSpeech` object.
 */
var OutputSpeech = /** @class */ (function() {
  /**
   * Creates an instance of OutputSpeech.
   *
   * @param {string} type
   * A string containing the type of output speech to render. Valid types are:
   * * "PlainText": Indicates that the output speech is defined as plain text.
   * * "SSML": Indicates that the output speech is text marked up with SSML.
   *
   * @param {string} [text]
   * A string containing the speech to render to the user. Use this when type is "PlainText".
   *
   * @param {string} [ssml]
   * A string containing text marked up with SSML to render to the user. Use this when type is  "SSML"
   *
   * @memberof OutputSpeech
   */
  function OutputSpeech(type, text, ssml) {
    if (!type) throw new Error("Invalid output speech type.");
    this.type = type;
    if (type === "PlainText") {
      if (!text) throw new Error("Type is PlainText but no plain text is present.");
      this.text = text;
    } else if (type === "SSML") {
      if (!ssml) throw new Error("Type is SSML but no ssml is present.");
      this.ssml = ssml;
    }
  }
  return OutputSpeech;
})();
/**
 * The `Card` Object.
 * This object can only be included when sending a response to a LaunchRequest or IntentRequest.
 */
var Card = /** @class */ (function() {
  /**
   * Creates an instance of Card.
   *
   *  @param {string} type
   * * A string describing the type of card to render. Valid types are:
   * * "Simple": A card that contains a title and plain text content.
   * * "Standard": A card that contains a title, text content, and an image to display.
   * * "LinkAccount": a card that displays a link to an authorization URL that the user can use to link their Alexa account with a user in another system. See Linking an Alexa User with a User in Your System for details.
   *
   * @param {string} [title]
   * A string containing the title of the card. (not applicable for cards of type LinkAccount).
   *
   * @param {string} [content]
   * A string containing the contents of a Simple card (not applicable for cards of type Standard or LinkAccount).
   *
   * @param {string} [text]
   * A string containing the text content for a Standard card (not applicable for cards of type Simple or LinkAccount)
   *
   * @param {*} [image]
   * An image object that specifies the URLs for the image to display on a Standard card. Only applicable for Standard cards.
   * You can provide two URLs, for use on different sized screens.
   * * smallImageUrl
   * * largeImageUrl
   *
   * See [Including a Card in Your Skill’s Response](https://developer.amazon.com/docs/custom-skills/include-a-card-in-your-skills-response.html)
   *
   * @memberof Card
   */
  function Card(type, title, content, text, image) {
    if (!type) throw new Error("Missing card type.");
    this.type = type;
    this.title = title;
    this.content = content;
    this.text = text;
    this.image = image;
  }
  return Card;
})();
/**
 * The `Reprompt` Object.
 * This object can only be included when sending a response to a LaunchRequest or IntentRequest.
 */
var Reprompt = /** @class */ (function() {
  /**
   * Creates an instance of Reprompt.
   *
   *  @param {*} [outputSpeech]
   * An OutputSpeech object containing the text or SSML to render as a re-prompt.
   *
   * @memberof Reprompt
   */
  function Reprompt(outputSpeech) {
    this.outputSpeech = outputSpeech;
  }
  return Reprompt;
})();
/**
 * The `Response` object.
 */
var Response = /** @class */ (function() {
  /**
   * Creates an instance of Response.
   *
   * @param {OutputSpeech} [outputSpeech]
   * The object containing the speech to render to the user. See `OutputSpeech` Object.
   *
   * @param {Card} [card]
   * The object containing a card to render to the Amazon Alexa App. See `Card` Object.
   *
   * @param {Reprompt} [reprompt]
   * The object containing the outputSpeech to use if a re-prompt is necessary.
   *
   * This is used if the your service keeps the session open after sending the response, but the user does not respond with anything that maps to an intent defined in your voice interface while the audio stream is open.
   *
   * If this is not set, the user is not re-prompted.
   *
   * @param {boolean} [shouldEndSession]
   * A boolean value with `true` meaning that the session should end after Alexa speaks the response, or `false` if the session should remain active.
   *
   * If not provided, defaults to `true`.
   *
   * @param {Array<any>} [directives]
   * An array of directives specifying device-level actions to take using a particular interface, such as the AudioPlayer interface for streaming audio.
   * For details about the directives you can include in your response, see:
   * * [AudioPlayer Interface Reference](https://developer.amazon.com/docs/custom-skills/audioplayer-interface-reference.html)
   * * [VideoApp Interface Reference](https://developer.amazon.com/docs/custom-skills/videoapp-interface-reference.html)
   * * [Display Interface Reference](https://developer.amazon.com/docs/custom-skills/display-interface-reference.html)
   * * [Dialog Interface Reference](https://developer.amazon.com/docs/custom-skills/dialog-interface-reference.html)
   *
   * @memberof Response
   */
  function Response(outputSpeech, card, reprompt, shouldEndSession, directives) {
    this.outputSpeech = outputSpeech;
    this.card = card;
    this.reprompt = reprompt;
    this.shouldEndSession = shouldEndSession;
    this.directives = directives;
  }
  return Response;
})();
/**
 * The response from STOCOBS skill.
 */
var StocObsResponse = /** @class */ (function() {
  /**
   * Creates an instance of StocObsResponse.
   *
   * @param {string} outputText
   * The output message that Alexa will tell to the user.
   *
   * @memberof StocObsResponse
   */
  function StocObsResponse(outputText) {
    this.version = "0.1.0";
    this.response = new Response(new OutputSpeech("PlainText", outputText));
  }
  return StocObsResponse;
})();
// =========================================================================================
//                                    HANDLER
// =========================================================================================
/**
 * The STOCOBS handler.
 *
 * @param {IStocObsEvent} event
 * The event that has triggered this handler.
 *
 * @param {*} context
 * The context of the event.
 *
 * @param {*} callback
 * The callback function that needs to be invoked after handling the event.
 */
var stocobsHandler = function(event, context, callback) {
  try {
    /////////////////////////////////////////////////////////////////////////////////
    if (event.request.type === "IntentRequest") console.log("Valid Intent Request!");
    else
      return callback(
        null,
        new StocObsResponse(
          "Try saying something like, Alexa, ask stock aubs about Microsoft.\n          Please note, Stock aubs only deals with companies listed on NASDAQ. It uses Alphavantage's Realtime Stock API. The time zone is US Eastern."
        )
      );
    /////////////////////////////////////////////////////////////////////////////////
    var companyNameSlot = event.request.intent.slots.companyName;
    var slotName = companyNameSlot.name;
    var companyName = companyNameSlot.value;
    console.log(">>>>>>> HARMLESS :: Alexa found company name to be " + companyName);
    getStockForCompany(companyName, callback); // Fetch stocks doing API call.
  } catch (error) {
    return callback(
      null,
      new StocObsResponse(
        "Try saying something like, Alexa, ask stock aubs about Microsoft.\n        Please note, Stock aubs only deals with companies listed on NASDAQ. It uses Alphavantage's Realtime Stock API. The time zone is US Eastern."
      )
    );
  }
};
/**
 * Fetches the stock of the company and constructs the message for the user.
 *
 * @param {string} companyName
 * The company whose stock the user needs.
 *
 * @param {any} callback
 * The callback function to execute after fetching the stocks.
 */
var getStockForCompany = function(companyName, callback) {
  if (!companyName) throw new Error("Couldn't get the company name.");
  var companyTickerCode = getCompanyCodeFromName(companyName);
  console.log(">>>> HARMLESS:: Company Ticker Code or Symbol = " + companyTickerCode);
  if (!companyTickerCode || (companyTickerCode && companyTickerCode.length < 1))
    return callback(null, new StocObsResponse("The company named " + companyName + " is not listed on NASDAQ"));
  var url =
    ALPHAVANTAGE_API +
    "?function=TIME_SERIES_INTRADAY&symbol=" +
    companyTickerCode +
    "&interval=1min&outputsize=compact&apikey=" +
    API_KEY; // uses Alphavantage Realtime stock API
  console.log(">>>>>> HARMLESS:: API URL :: " + url);
  /////////////////////////// API CALL /////////////////////////////////////////////////////////////////////
  var req = https_1.get(url, function(res) {
    console.log(">>>> HARMLESS:: AlphaVantage's response code = " + res.statusCode);
    var chunks = [];
    res.on("data", function(chunk) {
      chunks.push(chunk);
    });
    res.on("end", function() {
      try {
        var bChunks = chunks.map(function(chunk) {
          return typeof chunk === "string" ? new buffer_1.Buffer(chunk) : chunk;
        });
        var body = buffer_1.Buffer.concat(bChunks);
        ////////////////// PARSE RESPONSE JSON BODY ////////////////////////////////
        var alphavantageResponse = JSON.parse(body.toString());
        ////////////////// PARSE RESPONSE JSON BODY ////////////////////////////////
        console.log(">>>>>> HARMLESS :: Alphavantage Response = " + JSON.stringify(alphavantageResponse));
        console.log(">>>>>> HARMLESS :: Alphavantage Data = " + alphavantageResponse["Time Series (1min)"]);
        if (!alphavantageResponse["Time Series (1min)"])
          return callback(
            null,
            new StocObsResponse(
              "Sorry, stock aubs couldn't get the stocks for " +
                companyName +
                " at the moment. Please try again after some time."
            )
          );
        else {
          /////////////////////// PROCESS RESPONSE /////////////////////////////////
          var timestamp = Object.getOwnPropertyNames(alphavantageResponse["Time Series (1min)"])[0];
          var stockData = alphavantageResponse["Time Series (1min)"][timestamp];
          var msg =
            "Stock aubs found that on " +
            timestamp +
            " the " +
            companyName +
            " stock opened at " +
            stockData["1. open"] +
            " USD.\n          \n          It had a high of " +
            stockData["2. high"] +
            " USD, a low of " +
            stockData["3. low"] +
            " USD, and closed at " +
            stockData["4. close"] +
            " USD. \n          \n          The volume was " +
            stockData["5. volume"] +
            " units.\n          \n          Please note that the time zone is always US Eastern.";
          return callback(null, new StocObsResponse(msg));
          /////////////////////// PROCESS RESPONSE /////////////////////////////////
        }
      } catch (error) {
        return callback(
          null,
          new StocObsResponse(
            "Try saying something like, Alexa, ask stock aubs about Microsoft.\n        Please note, Stock aubs only deals with companies listed on NASDAQ. It uses Alphavantage's Realtime Stock API. The time zone is US Eastern."
          )
        );
      }
    });
  });
  /////////////////////////// API CALL /////////////////////////////////////////////////////////////////////
};
/**
 * Fetches yesterday's date in `yyyy-mm-dd` format.
 * @returns {string} The date in `yyyy-mm-dd` format.
 */
var getYesterdayDate = function() {
  var date = new Date();
  date.setDate(date.getDate() - 1); // get yesterday's date
  return date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate();
};
/**
 * Fetches the ticker code of the company from its name.
 *
 * @param {string} companyName
 * The name of the company for which we need the ticker code.
 *
 * @returns {string} The ticker code of the company.
 */
var getCompanyCodeFromName = function(companyName) {
  for (var _i = 0, _a = Object.getOwnPropertyNames(COMPANY_TICKER_MAP); _i < _a.length; _i++) {
    var key = _a[_i];
    if (key.toLowerCase().indexOf(companyName.toLowerCase()) !== -1) return COMPANY_TICKER_MAP[key];
  }
  return "";
};
/**
 * Alphavantage API for stocks
 */
var ALPHAVANTAGE_API = "https://www.alphavantage.co/query";
/**
 * Personal API Key.
 */
// const API_KEY: string = "BPszsAzY2t7AR5YBh7kw";
// API key for Alpha Vantage -- Realtime stock API
var API_KEY = "3OB0J5INWKH5BG1W";
/**
 * The COMPANY name to their TICKER code mapping for NASDAQ.
 */
var COMPANY_TICKER_MAP = {
  "Advanced Accelerator Applications S.A.": "AAAP",
  "Altaba Inc.": "AABA",
  "American Airlines Group Inc.": "AAL",
  "Atlantic American Corporation": "AAME",
  "Applied Optoelectronics Inc.": "AAOI",
  "AAON Inc.": "AAON",
  "Apple Inc.": "AAPL",
  "Atlas Air Worldwide Holdings": "AAWW",
  "iShares MSCI All Country Asia ex Japan Index Fund": "AAXJ",
  "Axon Enterprise Inc.": "AAXN",
  "Renmin Tianli Group Inc.": "ABAC",
  "ABAXIS Inc.": "ABAX",
  "Ameris Bancorp": "ABCB",
  "Cambium Learning Group Inc.": "ABCD",
  "Alcentra Capital Corp.": "ABDC",
  "Abeona Therapeutics Inc.": "ABEO",
  "Abeona Therapeutics Inc. Warrant": "ABEOW",
  "Ability Inc.": "ABIL",
  "ARCA biopharma Inc.": "ABIO",
  "Ablynx NV": "ABLX",
  "ABIOMED Inc.": "ABMD",
  "Allegiance Bancshares Inc.": "ABTX",
  "Arbutus Biopharma Corporation": "ABUS",
  "ACADIA Pharmaceuticals Inc.": "ACAD",
  "Atlantic Capital Bancshares Inc.": "ACBI",
  "Acer Therapeutics Inc.": "ACER",
  "Acer Therapeutics Inc. Warrant": "ACERW",
  "Aceto Corporation": "ACET",
  "Atlantic Coast Financial Corporation": "ACFC",
  "Arch Capital Group Ltd.": "ACGL",
  "Arch Capital Group Ltd. Depositary Shares Each Representing 1/1000th Interest in a Share of5.45% Non-Cumulative Preferred Shares Series F":
    "ACGLO",
  "Arch Capital Group Ltd. Depositary Shares Representing Interest in 5.25% Non-Cumulative Preferred Series E Shrs":
    "ACGLP",
  "Acadia Healthcare Company Inc.": "ACHC",
  "Achillion Pharmaceuticals Inc.": "ACHN",
  "Achieve Life Sciences Inc.": "ACHV",
  "Acacia Communications Inc.": "ACIA",
  "AC Immune SA": "ACIU",
  "ACI Worldwide Inc.": "ACIW",
  "Axcelis Technologies Inc.": "ACLS",
  "ACM Research Inc.": "ACMR",
  "ACNB Corporation": "ACNB",
  "Acorda Therapeutics Inc.": "ACOR",
  "Aclaris Therapeutics Inc.": "ACRS",
  "AcelRx Pharmaceuticals Inc.": "ACRX",
  "American Capital Senior Floating Ltd.": "ACSF",
  "Acasti Pharma Inc.": "ACST",
  "AdvisorShares Vice ETF": "ACT",
  "Actua Corporation": "ACTA",
  "Acacia Research Corporation": "ACTG",
  "iShares MSCI ACWI Index Fund": "ACWI",
  "iShares MSCI ACWI ex US Index Fund": "ACWX",
  "Acxiom Corporation": "ACXM",
  "Adaptimmune Therapeutics plc": "ADAP",
  "Adobe Systems Incorporated": "ADBE",
  "Advanced Emissions Solutions Inc.": "ADES",
  "Analog Devices Inc.": "ADI",
  "ADMA Biologics Inc": "ADMA",
  "Adamis Pharmaceuticals Corporation": "ADMP",
  "Adamas Pharmaceuticals Inc.": "ADMS",
  "ADOMANI Inc.": "ADOM",
  "Automatic Data Processing Inc.": "ADP",
  "BLDRS Asia 50 ADR Index Fund": "ADRA",
  "BLDRS Developed Markets 100 ADR Index Fund": "ADRD",
  "BLDRS Emerging Markets 50 ADR Index Fund": "ADRE",
  "Aduro Biotech Inc.": "ADRO",
  "BLDRS Europe 100 ADR Index Fund": "ADRU",
  "Autodesk Inc.": "ADSK",
  "ADTRAN Inc.": "ADTN",
  "Addus HomeCare Corporation": "ADUS",
  "Adverum Biotechnologies Inc.": "ADVM",
  "Advaxis Inc.": "ADXS",
  "Advaxis Inc. Warrants": "ADXSW",
  "Aegion Corp": "AEGN",
  "Aehr Test Systems": "AEHR",
  "Advanced Energy Industries Inc.": "AEIS",
  "Aethlon Medical Inc.": "AEMD",
  "Aerie Pharmaceuticals Inc.": "AERI",
  "American Electric Technologies Inc.": "AETI",
  "ADDvantage Technologies Group Inc.": "AEY",
  "Aeterna Zentaris Inc.": "AEZS",
  "Almost Family Inc": "AFAM",
  "Atlas Financial Holdings Inc.": "AFH",
  "Atlas Financial Holdings Inc. 6.625% Senior Unsecured Notes Due 2022": "AFHBL",
  "Affimed N.V.": "AFMD",
  "AmTrust Financial Services Inc.": "AFSI",
  "Agenus Inc.": "AGEN",
  "AgroFresh Solutions Inc.": "AGFS",
  "AgroFresh Solutions Inc. Warrants": "AGFSW",
  "Argo Group International Holdings Ltd.": "AGII",
  "Argo Group International Holdings Ltd. 6.5% Senior Notes Due 2042": "AGIIL",
  "Agios Pharmaceuticals Inc.": "AGIO",
  "Aeglea BioTherapeutics Inc.": "AGLE",
  "AGNC Investment Corp.": "AGNC",
  "AGNC Investment Corp. Depositary Shares representing 1/1000th Series B Preferred Stock": "AGNCB",
  "AGNC Investment Corp. Depositary Shares Each Representing a 1/1000th Interest in a Share of 7.00% Series C Fixed-To-Floating Rate Cumulative Redeemable Preferre":
    "AGNCN",
  "WisdomTree Barclays Negative Duration U.S. Aggregate Bond Fund": "AGND",
  "Agile Therapeutics Inc.": "AGRX",
  "Applied Genetic Technologies Corporation": "AGTC",
  "Agilysys Inc.": "AGYS",
  "WisdomTree Barclays Interest Rate Hedged U.S. Aggregate Bond Fund": "AGZD",
  "Alliance Holdings GP L.P.": "AHGP",
  "Avista Healthcare Public Acquisition Corp.": "AHPA",
  "Avista Healthcare Public Acquisition Corp. Unit": "AHPAU",
  "Avista Healthcare Public Acquisition Corp. Warrants": "AHPAW",
  "Allied Healthcare Products Inc.": "AHPI",
  "Altra Industrial Motion Corp.": "AIMC",
  "Aimmune Therapeutics Inc.": "AIMT",
  "Apollo Investment Corporation": "AINV",
  "Airgain Inc.": "AIRG",
  "First Trust RBA American Industrial Renaissance ETF": "AIRR",
  "Air T Inc.": "AIRT",
  "Akamai Technologies Inc.": "AKAM",
  "Achaogen Inc.": "AKAO",
  "Akebia Therapeutics Inc.": "AKBA",
  "Akcea Therapeutics Inc.": "AKCA",
  "Akers Biosciences Inc": "AKER",
  "Akorn Inc.": "AKRX",
  "Akoustis Technologies Inc.": "AKTS",
  "Akari Therapeutics Plc": "AKTX",
  "Albireo Pharma Inc.": "ALBO",
  "Alico Inc.": "ALCO",
  "Alder BioPharmaceuticals Inc.": "ALDR",
  "Aldeyra Therapeutics Inc.": "ALDX",
  "Align Technology Inc.": "ALGN",
  "Allegiant Travel Company": "ALGT",
  "Alimera Sciences Inc.": "ALIM",
  "ALJ Regional Holdings Inc.": "ALJJ",
  "Alkermes plc": "ALKS",
  "Allot Communications Ltd.": "ALLT",
  "Allena Pharmaceuticals Inc.": "ALNA",
  "Alnylam Pharmaceuticals Inc.": "ALNY",
  "Analogic Corporation": "ALOG",
  "AstroNova Inc.": "ALOT",
  "Alpine Immune Sciences Inc.": "ALPN",
  "Alliqua BioMedical Inc.": "ALQA",
  "Alarm.com Holdings Inc.": "ALRM",
  "Aileron Therapeutics Inc.": "ALRN",
  "Alaska Communications Systems Group Inc.": "ALSK",
  "Altair Engineering Inc.": "ALTR",
  "Global X SuperDividend Alternatives ETF": "ALTY",
  "Alexion Pharmaceuticals Inc.": "ALXN",
  "AMAG Pharmaceuticals Inc.": "AMAG",
  "Applied Materials Inc.": "AMAT",
  "Ambarella Inc.": "AMBA",
  "Ambac Financial Group Inc.": "AMBC",
  "Ambac Financial Group Inc. Warrants": "AMBCW",
  "iShares Russell 1000 Pure U.S. Revenue ETF": "AMCA",
  "AirMedia Group Inc": "AMCN",
  "AMC Networks Inc.": "AMCX",
  "Amedica Corporation": "AMDA",
  "Amedisys Inc": "AMED",
  "Apollo Medical Holdings Inc.": "AMEH",
  "Amgen Inc.": "AMGN",
  "Amkor Technology Inc.": "AMKR",
  "Alliance MMA Inc.": "AMMA",
  "American National Bankshares Inc.": "AMNB",
  "Allied Motion Technologies Inc.": "AMOT",
  "America Movil S.A.B. de C.V. Class An Depositary Shares": "AMOV",
  "Amphastar Pharmaceuticals Inc.": "AMPH",
  "American River Bankshares": "AMRB",
  "Ameri Holdings Inc.": "AMRH",
  "Ameri Holdings Inc. Warrant": "AMRHW",
  "A-Mark Precious Metals Inc.": "AMRK",
  "Amarin Corporation plc": "AMRN",
  "Amyris Inc.": "AMRS",
  "American Superconductor Corporation": "AMSC",
  "AMERISAFE Inc.": "AMSF",
  "American Software Inc. Class A Common Stock": "AMSWA",
  "Aemetis Inc": "AMTX",
  "American Woodmark Corporation": "AMWD",
  "Amazon.com Inc.": "AMZN",
  "AnaptysBio Inc.": "ANAB",
  "American National Insurance Company": "ANAT",
  "Anchor Bancorp": "ANCB",
  "Access National Corporation": "ANCX",
  "Andina Acquisition Corp. II": "ANDA",
  "Andina Acquisition Corp. II Right": "ANDAR",
  "Andina Acquisition Corp. II Unit": "ANDAU",
  "Andina Acquisition Corp. II Warrant": "ANDAW",
  "The Andersons Inc.": "ANDE",
  "ANGI Homeservices Inc.": "ANGI",
  "AngioDynamics Inc.": "ANGO",
  "Anika Therapeutics Inc.": "ANIK",
  "ANI Pharmaceuticals Inc.": "ANIP",
  "ANSYS Inc.": "ANSS",
  "Anthera Pharmaceuticals Inc.": "ANTH",
  "Sphere 3D Corp.": "ANY",
  "American Outdoor Brands Corporation": "AOBC",
  "Alpha and Omega Semiconductor Limited": "AOSL",
  "Applied DNA Sciences Inc": "APDN",
  "Applied DNA Sciences Inc Warrant": "APDNW",
  "American Public Education Inc.": "APEI",
  "Apollo Endosurgery Inc.": "APEN",
  "Archrock Partners L.P.": "APLP",
  "Apellis Pharmaceuticals Inc.": "APLS",
  "Apogee Enterprises Inc.": "APOG",
  "Cellect Biotechnology Ltd.": "APOP",
  "Cellect Biotechnology Ltd. Warrants to Purchase ADR (1 WT and $7.50 to purchase 1 ADS)": "APOPW",
  "AppFolio Inc.": "APPF",
  "Appian Corporation": "APPN",
  "Digital Turbine Inc.": "APPS",
  "Apricus Biosciences Inc.": "APRI",
  "Apptio Inc.": "APTI",
  "Aptose Biosciences Inc.": "APTO",
  "Aptevo Therapeutics Inc.": "APVO",
  "Asia Pacific Wire & Cable Corporation Limited": "APWC",
  "AquaBounty Technologies Inc.": "AQB",
  "Aqua Metals Inc.": "AQMS",
  "Aquinox Pharmaceuticals Inc.": "AQXP",
  "Accuray Incorporated": "ARAY",
  "ArcBest Corporation": "ARCB",
  "Ares Capital Corporation": "ARCC",
  "Appliance Recycling Centers of America Inc.": "ARCI",
  "Arcturus Therapeutics Ltd.": "ARCT",
  "ARC Group Worldwide Inc.": "ARCW",
  "Aradigm Corporation": "ARDM",
  "Ardelyx Inc.": "ARDX",
  "Approach Resources Inc.": "AREX",
  "Argos Therapeutics Inc.": "ARGS",
  "argenx SE": "ARGX",
  "American Railcar Industries Inc.": "ARII",
  "Ark Restaurants Corp.": "ARKR",
  "Alliance Resource Partners L.P.": "ARLP",
  "Aralez Pharmaceuticals Inc.": "ARLZ",
  "Arena Pharmaceuticals Inc.": "ARNA",
  "Arrow Financial Corporation": "AROW",
  "ArQule Inc.": "ARQL",
  "ARRIS International plc": "ARRS",
  "Array BioPharma Inc.": "ARRY",
  "Artesian Resources Corporation Class A Non-Voting Common Stock": "ARTNA",
  "Art's-Way Manufacturing Co. Inc.": "ARTW",
  "Arotech Corporation": "ARTX",
  "Arrowhead Pharmaceuticals Inc.": "ARWR",
  "Associated Banc-Corp": "ASB",
  "Ascent Capital Group Inc. Series A Common Stock": "ASCMA",
  "FlexShares Real Assets Allocation Index Fund": "ASET",
  "Asta Funding Inc.": "ASFI",
  "Assembly Biosciences Inc.": "ASMB",
  "ASML Holding N.V.": "ASML",
  "Ascena Retail Group Inc.": "ASNA",
  "Ascendis Pharma A/S": "ASND",
  "Arsanis Inc.": "ASNS",
  "Altisource Portfolio Solutions S.A.": "ASPS",
  "Aspen Group Inc.": "ASPU",
  "AmeriServ Financial Inc.": "ASRV",
  "AmeriServ Financial Inc. AmeriServ Financial Trust I": "ASRVP",
  "Astrotech Corporation": "ASTC",
  "Astec Industries Inc.": "ASTE",
  "Asure Software Inc": "ASUR",
  "ASV Holdings Inc.": "ASV",
  "Amtech Systems Inc.": "ASYS",
  "Atlantic Acquisition Corp.": "ATAC",
  "Atlantic Acquisition Corp. Right": "ATACR",
  "Atlantic Acquisition Corp. Unit": "ATACU",
  "ATA Inc.": "ATAI",
  "America First Multifamily Investors L.P.": "ATAX",
  "Alphatec Holdings Inc.": "ATEC",
  "athenahealth Inc.": "ATHN",
  "Athersys Inc.": "ATHX",
  "Atlanticus Holdings Corporation": "ATLC",
  "Ames National Corporation": "ATLO",
  "ATN International Inc.": "ATNI",
  "Athenex Inc.": "ATNX",
  "Atomera Incorporated": "ATOM",
  "Atossa Genetics Inc.": "ATOS",
  "Atara Biotherapeutics Inc.": "ATRA",
  "AtriCure Inc.": "ATRC",
  "Atrion Corporation": "ATRI",
  "Astronics Corporation": "ATRO",
  "Antares Pharma Inc.": "ATRS",
  "Air Transport Services Group Inc": "ATSG",
  "Attunity Ltd.": "ATTU",
  "Activision Blizzard Inc": "ATVI",
  "Avenue Therapeutics Inc.": "ATXI",
  "Auburn National Bancorporation Inc.": "AUBN",
  "AudioCodes Ltd.": "AUDC",
  "Aurinia Pharmaceuticals Inc": "AUPH",
  "AutoWeb Inc.": "AUTO",
  "AeroVironment Inc.": "AVAV",
  "Avadel Pharmaceuticals plc": "AVDL",
  "AVEO Pharmaceuticals Inc.": "AVEO",
  "Broadcom Limited": "AVGO",
  "Avinger Inc.": "AVGR",
  "A V Homes Inc.": "AVHI",
  "Avid Technology Inc.": "AVID",
  "Aviragen Therapeutics Inc.": "AVIR",
  "Aviat Networks Inc.": "AVNW",
  "Anavex Life Sciences Corp.": "AVXL",
  "AveXis Inc.": "AVXS",
  "Aware Inc.": "AWRE",
  "Abraxas Petroleum Corporation": "AXAS",
  "Accelerate Diagnostics Inc.": "AXDX",
  "AxoGen Inc.": "AXGN",
  "Axsome Therapeutics Inc.": "AXSM",
  "AXT Inc": "AXTI",
  "Atlantica Yield plc": "AY",
  "Aytu BioScience Inc.": "AYTU",
  "Aspen Technology Inc.": "AZPN",
  "AzurRx BioPharma Inc.": "AZRX",
  "Natus Medical Incorporated": "BABY",
  "Bandwidth Inc.": "BAND",
  "BancFirst Corporation": "BANF",
  "BancFirst Corporation 7.2% Cumulative Trust Preferred Securities": "BANFP",
  "Banner Corporation": "BANR",
  "StoneCastle Financial Corp": "BANX",
  "Bioanalytical Systems Inc.": "BASI",
  "Liberty Media Corporation Series A Liberty Braves Common Stock": "BATRA",
  "Liberty Media Corporation Series C Liberty Braves Common Stock": "BATRK",
  "BlackBerry Limited": "BB",
  "Bed Bath & Beyond Inc.": "BBBY",
  "Virtus LifeSci Biotech Clinical Trials": "BBC",
  "Beasley Broadcast Group Inc.": "BBGI",
  "Black Box Corporation": "BBOX",
  "Virtus LifeSci Biotech Products": "BBP",
  "Bravo Brio Restaurant Group Inc.": "BBRG",
  "Barrett Business Services Inc.": "BBSI",
  "Bison Capital Acquisition Corp.": "BCAC",
  "Bison Capital Acquisition Corp. Rights": "BCACR",
  "Bison Capital Acquisition Corp. Units Consisting of 1 OS 1/2 WT and 1 RT": "BCACU",
  "Bison Capital Acquisition Corp. Warrant": "BCACW",
  "BCB Bancorp Inc. (NJ)": "BCBP",
  "Brainstorm Cell Therapeutics Inc.": "BCLI",
  "B Communications Ltd.": "BCOM",
  "Blucora Inc.": "BCOR",
  "Brightcove Inc.": "BCOV",
  "Balchem Corporation": "BCPC",
  "BioCryst Pharmaceuticals Inc.": "BCRX",
  "Bancorp 34 Inc.": "BCTF",
  "Bridge Bancorp Inc.": "BDGE",
  "BioDelivery Sciences International Inc.": "BDSI",
  "BioTelemetry Inc.": "BEAT",
  "bebe stores inc.": "BEBE",
  "Beacon Roofing Supply Inc.": "BECN",
  "Bel Fuse Inc. Class A Common Stock": "BELFA",
  "Bel Fuse Inc. Class B Common Stock": "BELFB",
  "BankFinancial Corporation": "BFIN",
  "Global X Health & Wellness Thematic ETF": "BFIT",
  "BGC Partners Inc.": "BGCP",
  "Big 5 Sporting Goods Corporation": "BGFV",
  "BeiGene Ltd.": "BGNE",
  "Barington/Hilco Acquisition Corp.": "BHAC",
  "Barington/Hilco Acquisition Corp. Right": "BHACR",
  "Barington/Hilco Acquisition Corp. Unit": "BHACU",
  "Barington/Hilco Acquisition Corp. Warrants": "BHACW",
  "Blue Hills Bancorp Inc.": "BHBK",
  "Brighthouse Financial Inc.": "BHF",
  "ProShares Ultra Nasdaq Biotechnology": "BIB",
  "First Trust BICK Index Fund": "BICK",
  "Baidu Inc.": "BIDU",
  "Biogen Inc.": "BIIB",
  "Biocept Inc.": "BIOC",
  "Biolase Inc.": "BIOL",
  "BioScrip Inc.": "BIOS",
  "ProShares UltraShort Nasdaq Biotechnology": "BIS",
  "Bioverativ Inc.": "BIVV",
  "BJ's Restaurants Inc.": "BJRI",
  "BlackRock Capital Investment Corporation": "BKCC",
  "Blueknight Energy Partners L.P. L.L.C.": "BKEP",
  "Blueknight Energy Partners L.P. L.L.C. Series A Preferred Units": "BKEPP",
  "Bank Mutual Corporation": "BKMU",
  "Bank of South Carolina Corp.": "BKSC",
  "BIO-key International Inc.": "BKYI",
  "BlackLine Inc.": "BL",
  "Blue Bird Corporation": "BLBD",
  "Bellicum Pharmaceuticals Inc.": "BLCM",
  "Reality Shares Nasdaq NextGen Economy ETF": "BLCN",
  "Ballard Power Systems Inc.": "BLDP",
  "Builders FirstSource Inc.": "BLDR",
  "BioLife Solutions Inc.": "BLFS",
  "Bridgeline Digital Inc.": "BLIN",
  "Blackbaud Inc.": "BLKB",
  "Bloomin' Brands Inc.": "BLMN",
  "BSB Bancorp Inc.": "BLMT",
  "Bellerophon Therapeutics Inc.": "BLPH",
  "BioLineRx Ltd.": "BLRX",
  "bluebird bio Inc.": "BLUE",
  "Boulevard Acquisition Corp. II": "BLVD",
  "Boulevard Acquisition Corp. II Unit": "BLVDU",
  "Boulevard Acquisition Corp. II Warrant": "BLVDW",
  "BMC Stock Holdings Inc.": "BMCH",
  "BMO Elkhorn DWA MLP Select Index Exchange Traded Notes due December 10 2036": "BMLP",
  "Biomerica Inc.": "BMRA",
  "Bank of Marin Bancorp": "BMRC",
  "BioMarin Pharmaceutical Inc.": "BMRN",
  "Bryn Mawr Bank Corporation": "BMTC",
  "Beneficial Bancorp Inc.": "BNCL",
  "Vanguard Total International Bond ETF": "BNDX",
  "Benefitfocus Inc.": "BNFT",
  "Bonso Electronics International Inc.": "BNSO",
  "Benitec Biopharma Limited": "BNTC",
  "Benitec Biopharma Limited Warrant": "BNTCW",
  "Bob Evans Farms Inc.": "BOBE",
  "Bank of Commerce Holdings (CA)": "BOCH",
  "BofI Holding Inc.": "BOFI",
  "BofI Holding Inc. 6.25% Subordinated Notes Due 2026": "BOFIL",
  "Bojangles' Inc.": "BOJA",
  "BOK Financial Corporation": "BOKF",
  "BOK Financial Corporation 5.375% Subordinated Notes due 2056": "BOKFL",
  "Audentes Therapeutics Inc.": "BOLD",
  "Boston Omaha Corporation": "BOMN",
  "DMC Global Inc.": "BOOM",
  "B.O.S. Better Online Solutions": "BOSC",
  "Bank of the James Financial Group Inc.": "BOTJ",
  "Global X Robotics & Artificial Intelligence ETF": "BOTZ",
  "Boxlight Corporation": "BOXL",
  "Boston Private Financial Holdings Inc.": "BPFH",
  "Boston Private Financial Holdings Inc. Depositary Shares representing 1/40th Interest in a Share of 6.95% Non-Cumulative Perpetual Preferred Stock Series D":
    "BPFHP",
  "Boston Private Financial Holdings Inc. Warrants to purchase 1 share of common stock @ $8.00/share": "BPFHW",
  "Blueprint Medicines Corporation": "BPMC",
  "Popular Inc.": "BPOP",
  "Popular Inc. Popular Capital Trust II": "BPOPM",
  "Popular Inc. Popular Capital Trust I -6.70% Cumulative Monthly Income Trust Preferred Securities": "BPOPN",
  "The Bank of Princeton": "BPRN",
  "Bio-Path Holdings Inc.": "BPTH",
  "Black Ridge Acquisition Corp.": "BRAC",
  "Black Ridge Acquisition Corp. Right": "BRACR",
  "Black Ridge Acquisition Corp. Unit": "BRACU",
  "Black Ridge Acquisition Corp. Warrant": "BRACW",
  "Craft Brew Alliance Inc.": "BREW",
  "Bridgford Foods Corporation": "BRID",
  "Brookline Bancorp Inc.": "BRKL",
  "Bruker Corporation": "BRKR",
  "Brooks Automation Inc.": "BRKS",
  "Big Rock Partners Acquisition Corp.": "BRPA",
  "Big Rock Partners Acquisition Corp. Right": "BRPAR",
  "Big Rock Partners Acquisition Corp. Unit": "BRPAU",
  "Big Rock Partners Acquisition Corp. Warrant": "BRPAW",
  "Borqs Technologies Inc.": "BRQS",
  "Borqs Technologies Inc. Warrants": "BRQSW",
  "Bassett Furniture Industries Incorporated": "BSET",
  "Bear State Financial Inc.": "BSF",
  "BroadSoft Inc.": "BSFT",
  "Biostar Pharmaceuticals Inc.": "BSPM",
  "BSQUARE Corporation": "BSQR",
  "Sierra Bancorp": "BSRR",
  "BioSpecifics Technologies Corp": "BSTC",
  "Principal Healthcare Innovators Index ETF": "BTEC",
  "Blue Buffalo Pet Products Inc.": "BUFF",
  "Burcon NutraScience Corp": "BUR",
  "Chanticleer Holdings Inc.": "BURG",
  "First Busey Corporation": "BUSE",
  "Bazaarvoice Inc.": "BV",
  "BroadVision Inc.": "BVSN",
  "BiondVax Pharmaceuticals Ltd.": "BVXV",
  "BiondVax Pharmaceuticals Ltd. Warrants": "BVXVW",
  "Broadwind Energy Inc.": "BWEN",
  "Bankwell Financial Group Inc.": "BWFG",
  "Baldwin & Lyons Inc. Class A (voting) Common Stock": "BWINA",
  "Baldwin & Lyons Inc. Class B (nonvoting) Common Stock": "BWINB",
  "Buffalo Wild Wings Inc.": "BWLD",
  "Bay Bancorp Inc.": "BYBK",
  "Broadway Financial Corporation": "BYFC",
  "BeyondSpring Inc.": "BYSI",
  "Baozun Inc.": "BZUN",
  "CA Inc.": "CA",
  "China Automotive Systems Inc.": "CAAS",
  "Camden National Corporation": "CAC",
  "Credit Acceptance Corporation": "CACC",
  "ClearBridge All Cap Growth ETF": "CACG",
  "China Advanced Construction Materials Group Inc.": "CADC",
  "8point3 Energy Partners LP": "CAFD",
  "The Cheesecake Factory Incorporated": "CAKE",
  "Calithera Biosciences Inc.": "CALA",
  "Callidus Software Inc.": "CALD",
  "China Auto Logistics Inc.": "CALI",
  "magicJack VocalTec Ltd": "CALL",
  "Cal-Maine Foods Inc.": "CALM",
  "CalAmp Corp.": "CAMP",
  "Camtek Ltd.": "CAMT",
  "Capricor Therapeutics Inc.": "CAPR",
  "Avis Budget Group Inc.": "CAR",
  "Cara Therapeutics Inc.": "CARA",
  "Carbonite Inc.": "CARB",
  "CarGurus Inc.": "CARG",
  "Carolina Financial Corporation": "CARO",
  "Carolina Trust BancShares Inc.": "CART",
  "Carver Bancorp Inc.": "CARV",
  "First Trust NASDAQ Global Auto Index Fund": "CARZ",
  "Casa Systems Inc.": "CASA",
  "Cascadian Therapeutics Inc.": "CASC",
  "Meta Financial Group Inc.": "CASH",
  "CASI Pharmaceuticals Inc.": "CASI",
  "CAS Medical Systems Inc.": "CASM",
  "Cass Information Systems Inc": "CASS",
  "Caseys General Stores Inc.": "CASY",
  "Catabasis Pharmaceuticals Inc.": "CATB",
  "Cambridge Bancorp": "CATC",
  "Global X S&P 500 Catholic Values ETF": "CATH",
  "Cardtronics plc": "CATM",
  "Catasys Inc.": "CATS",
  "Cathay General Bancorp": "CATY",
  "Cathay General Bancorp Warrant": "CATYW",
  "Cavium Inc.": "CAVM",
  "CBAK Energy Technology Inc.": "CBAK",
  "Colony Bankcorp Inc.": "CBAN",
  "CymaBay Therapeutics Inc.": "CBAY",
  "Capital Bank Financial Corp.": "CBF",
  "CB Financial Services Inc.": "CBFV",
  "Catalyst Biosciences Inc.": "CBIO",
  "Cleveland BioLabs Inc.": "CBLI",
  "Cellular Biomedicine Group Inc.": "CBMG",
  "Cboe Global Markets Inc.": "CBOE",
  "China Biologic Products Holdings Inc.": "CBPO",
  "Cracker Barrel Old Country Store Inc.": "CBRL",
  "Commerce Bancshares Inc.": "CBSH",
  "Commerce Bancshares Inc. Depositary Shares each representing a 1/1000th interest of 6.00% Series B Non-Cumulative Perpetual Preferred Stock":
    "CBSHP",
  "CBTX Inc.": "CBTX",
  "Capital City Bank Group": "CCBG",
  "China Ceramics Co. Ltd.": "CCCL",
  "China Commercial Credit Inc.": "CCCR",
  "Calamos Dynamic Convertible & Income Fund": "CCD",
  "ChinaCache International Holdings Ltd.": "CCIH",
  "CSI Compressco LP": "CCLP",
  "Cabot Microelectronics Corporation": "CCMP",
  "CNB Financial Corporation": "CCNE",
  "Cogent Communications Holdings Inc.": "CCOI",
  "China Customer Relations Centers Inc.": "CCRC",
  "Cross Country Healthcare Inc.": "CCRN",
  "CCUR Holdings Inc.": "CCUR",
  "ChemoCentryx Inc.": "CCXI",
  "VictoryShares US EQ Income Enhanced Volatility Wtd ETF": "CDC",
  "Centennial Resource Development Inc.": "CDEV",
  "CDK Global Inc.": "CDK",
  "VictoryShares US Large Cap High Div Volatility Wtd ETF": "CDL",
  "Avid Bioservices Inc.": "CDMO",
  "Avid Bioservices Inc. 10.50% Series E Convertible Preferred Stock": "CDMOP",
  "CareDx Inc.": "CDNA",
  "Cadence Design Systems Inc.": "CDNS",
  "Condor Hospitality Trust Inc.": "CDOR",
  "Clean Diesel Technologies Inc.": "CDTI",
  "Cidara Therapeutics Inc.": "CDTX",
  "CDW Corporation": "CDW",
  "ChromaDex Corporation": "CDXC",
  "Codexis Inc.": "CDXS",
  "Cadiz Inc.": "CDZI",
  "CECO Environmental Corp.": "CECE",
  "Career Education Corporation": "CECO",
  "Celcuity Inc.": "CELC",
  "Celgene Corporation": "CELG",
  "Celgene Corporation Contingent Value Right": "CELGZ",
  "Celsius Holdings Inc.": "CELH",
  "Chembio Diagnostics Inc.": "CEMI",
  "Central Garden & Pet Company": "CENT",
  "Central Garden & Pet Company Class A Common Stock Nonvoting": "CENTA",
  "Century Aluminum Company": "CENX",
  "Cerecor Inc.": "CERC",
  "Cerecor Inc. Class A Warrants": "CERCW",
  "Cerner Corporation": "CERN",
  "Cerus Corporation": "CERS",
  "Central European Media Enterprises Ltd.": "CETV",
  "Cemtrex Inc.": "CETX",
  "Cemtrex Inc. Series 1 Preferred Stock": "CETXP",
  "Cemtrex Inc. Series 1 Warrant": "CETXW",
  "CEVA Inc.": "CEVA",
  "VictoryShares Emerging Market High Div Volatility Wtd ETF": "CEY",
  "VictoryShares Emerging Market Volatility Wtd ETF": "CEZ",
  "VictoryShares US 500 Volatility Wtd ETF": "CFA",
  "Community First Bancshares Inc.": "CFBI",
  "Central Federal Corporation": "CFBK",
  "CF Corporation Unit": "CFCOU",
  "C&F Financial Corporation": "CFFI",
  "Capitol Federal Financial Inc.": "CFFN",
  "ConforMIS Inc.": "CFMS",
  "VictoryShares US 500 Enhanced Volatility Wtd ETF": "CFO",
  "ContraFect Corporation": "CFRX",
  "The Carlyle Group L.P.": "CG",
  "TCG BDC Inc.": "CGBD",
  "Compugen Ltd.": "CGEN",
  "Cancer Genetics Inc.": "CGIX",
  "Cogentix Medical Inc.": "CGNT",
  "Cognex Corporation": "CGNX",
  "Calamos Global Total Return Fund": "CGO",
  "Comstock Holding Companies Inc.": "CHCI",
  "City Holding Company": "CHCO",
  "Churchill Downs Incorporated": "CHDN",
  "The Chefs' Warehouse Inc.": "CHEF",
  "Check-Cap Ltd.": "CHEK",
  "Check-Cap Ltd. Series A Warrant": "CHEKW",
  "Chemical Financial Corporation": "CHFC",
  "Charter Financial Corp.": "CHFN",
  "CHF Solutions Inc.": "CHFS",
  "Calamos Convertible Opportunities and Income Fund": "CHI",
  "Cherokee Inc.": "CHKE",
  "Check Point Software Technologies Ltd.": "CHKP",
  "Chiasma Inc.": "CHMA",
  "Chemung Financial Corp": "CHMG",
  "China Natural Resources Inc.": "CHNR",
  "Coherus BioSciences Inc.": "CHRS",
  "C.H. Robinson Worldwide Inc.": "CHRW",
  "CHS Inc Class B Cumulative Redeemable Preferred Stock Series 4": "CHSCL",
  "CHS Inc Class B Reset Rate Cumulative Redeemable Preferred Stock Series 3": "CHSCM",
  "CHS Inc Preferred Class B Series 2 Reset Rate": "CHSCN",
  "CHS Inc Class B Cumulative Redeemable Preferred Stock": "CHSCO",
  "CHS Inc 8%  Cumulative Redeemable Preferred Stock": "CHSCP",
  "Charter Communications Inc.": "CHTR",
  "CommerceHub Inc. Series A Common Stock": "CHUBA",
  "CommerceHub Inc.": "CHUBK",
  "Chuy's Holdings Inc.": "CHUY",
  "Calamos Global Dynamic Income Fund": "CHW",
  "Calamos Convertible and High Income Fund": "CHY",
  "First Trust NASDAQ Cybersecurity ETF": "CIBR",
  "VictoryShares International High Div Volatility Wtd ETF": "CID",
  "Cinedigm Corp": "CIDM",
  "China Internet Nationwide Financial Services Inc.": "CIFS",
  "Colliers International Group Inc.": "CIGI",
  "VictoryShares International Volatility Wtd ETF": "CIL",
  "Cincinnati Financial Corporation": "CINF",
  "Cision Ltd.": "CISN",
  "Civista Bancshares Inc.": "CIVB",
  "Civista Bancshares Inc. Depositary Shares Each Representing a 1/40th Interest in a 6.50% Noncumulative Redeemable Convertible Perpetual Preferred Share Series B":
    "CIVBP",
  "VictoryShares Developed Enhanced Volatility Wtd ETF": "CIZ",
  "Citizens Holding Company": "CIZN",
  "China Jo-Jo Drugstores Inc.": "CJJD",
  "Checkpoint Therapeutics Inc.": "CKPT",
  "Clarus Corporation": "CLAR",
  "Caladrius Biosciences Inc.": "CLBS",
  "Collectors Universe Inc.": "CLCT",
  "China Lending Corporation": "CLDC",
  "Celldex Therapeutics Inc.": "CLDX",
  "Clearfield Inc.": "CLFD",
  "ClearSign Combustion Corporation": "CLIR",
  "ClearSign Combustion Corporation Warrant": "CLIRW",
  "Cellectis S.A.": "CLLS",
  "Calumet Specialty Products Partners L.P.": "CLMT",
  "Clean Energy Fuels Corp.": "CLNE",
  "Cleantech Solutions International Inc.": "CLNT",
  "Cellectar Biosciences Inc.": "CLRB",
  "Cellectar Biosciences Inc. Warrants": "CLRBW",
  "Cellectar Biosciences Inc. Series A Warrants": "CLRBZ",
  "IQ Chaikin U.S. Large Cap ETF": "CLRG",
  "ClearOne Inc.": "CLRO",
  "Clearside Biomedical Inc.": "CLSD",
  "Celsion Corporation": "CLSN",
  "Town Sports International Holdings Inc.": "CLUB",
  "Clovis Oncology Inc.": "CLVS",
  "Euro Tech Holdings Company Limited": "CLWT",
  "Calyxt Inc.": "CLXT",
  "Columbus McKinnon Corporation": "CMCO",
  "Comcast Corporation Class A Common Stock": "CMCSA",
  "CIM Commercial Trust Corporation": "CMCT",
  "CME Group Inc.": "CME",
  "CM Finance Inc": "CMFN",
  "Cimpress N.V": "CMPR",
  "Chimerix Inc.": "CMRX",
  "CM Seven Star Acquisition Corporation": "CMSS",
  "CM Seven Star Acquisition Corporation Right": "CMSSR",
  "CM Seven Star Acquisition Corporation Unit": "CMSSU",
  "CM Seven Star Acquisition Corporation Warrant": "CMSSW",
  "Clementia Pharmaceuticals Inc.": "CMTA",
  "Comtech Telecommunications Corp.": "CMTL",
  "Constellation Alpha Capital Corp.": "CNAC",
  "Constellation Alpha Capital Corp. Right": "CNACR",
  "Constellation Alpha Capital Corp. Unit": "CNACU",
  "Constellation Alpha Capital Corp. Warrant": "CNACW",
  "Conatus Pharmaceuticals Inc.": "CNAT",
  "Century Bancorp Inc. Class A Common Stock": "CNBKA",
  "Concert Pharmaceuticals Inc.": "CNCE",
  "Loncar Cancer Immunotherapy ETF": "CNCR",
  "ChinaNet Online Holdings Inc.": "CNET",
  "Conifer Holdings Inc.": "CNFR",
  "China Information Technology Inc.": "CNIT",
  "CONMED Corporation": "CNMD",
  "ConnectOne Bancorp Inc.": "CNOB",
  "Consolidated Communications Holdings Inc.": "CNSL",
  "China TechFaith Wireless Communication Technology Limited": "CNTF",
  "Century Casinos Inc.": "CNTY",
  "PC Connection Inc.": "CNXN",
  "CoBiz Financial Inc.": "COBZ",
  "Coda Octopus Group Inc.": "CODA",
  "Co-Diagnostics Inc.": "CODX",
  "Coherent Inc.": "COHR",
  "Cohu Inc.": "COHU",
  "Coca-Cola Bottling Co. Consolidated": "COKE",
  "Columbia Banking System Inc.": "COLB",
  "Collegium Pharmaceutical Inc.": "COLL",
  "Columbia Sportswear Company": "COLM",
  "CommScope Holding Company Inc.": "COMM",
  "iShares Commodities Select Strategy ETF": "COMT",
  "CyrusOne Inc": "CONE",
  "Conn's Inc.": "CONN",
  "PolarityTE Inc.": "COOL",
  "Core-Mark Holding Company Inc.": "CORE A",
  "Corium International Inc.": "CORI",
  "Corcept Therapeutics Incorporated": "CORT",
  "Costco Wholesale Corporation": "COST",
  "Cowen Inc.": "COWN",
  "Cowen Inc. 8.25% Senior Notes due 2021": "COWNL",
  "Cowen Inc. 7.35% Senior Notes Due 2027": "COWNZ",
  "CounterPath Corporation": "CPAH",
  "Canterbury Park Holding Corporation": "CPHC",
  "Cumberland Pharmaceuticals Inc.": "CPIX",
  "Capella Education Company": "CPLA",
  "Capital Product Partners L.P.": "CPLP",
  "Copart Inc.": "CPRT",
  "Catalyst Pharmaceuticals Inc.": "CPRX",
  "CPS Technologies Corp.": "CPSH",
  "Computer Programs and Systems Inc.": "CPSI",
  "Consumer Portfolio Services Inc.": "CPSS",
  "Capstone Turbine Corporation": "CPST",
  "Capitala Finance Corp.": "CPTA",
  "Capitala Finance Corp. 5.75% Convertible Notes Due 2022": "CPTAG",
  "Capitala Finance Corp. 6% Notes Due 2022": "CPTAL",
  "CRA InternationalInc.": "CRAI",
  "Cray Inc": "CRAY",
  "Corbus Pharmaceuticals Holdings Inc.": "CRBP",
  "Cree Inc.": "CREE",
  "China Recycling Energy Corporation": "CREG",
  "Cresud S.A.C.I.F. y A. American Depositary Shares each representing ten shares of Common Stock": "CRESY",
  "Curis Inc.": "CRIS",
  "Cardiome Pharma Corporation": "CRME",
  "America's Car-Mart Inc.": "CRMT",
  "Ceragon Networks Ltd.": "CRNT",
  "Crocs Inc.": "CROX",
  "CRISPR Therapeutics AG": "CRSP",
  "Criteo S.A.": "CRTO",
  "Cirrus Logic Inc.": "CRUS",
  "Calvert Ultra-Short Duration Income NextShares": "CRUSCNX",
  "CorVel Corp.": "CRVL",
  "Corvus Pharmaceuticals Inc.": "CRVS",
  "Crown Crafts Inc.": "CRWS",
  "Carrizo Oil & Gas Inc.": "CRZO",
  "VictoryShares US Small Cap High Div Volatility Wtd ETF": "CSB",
  "Clifton Bancorp Inc.": "CSBK",
  "Champions Oncology Inc.": "CSBR",
  "Cisco Systems Inc.": "CSCO",
  "VictoryShares US Discovery Enhanced Volatility Wtd ETF": "CSF",
  "CenterState Bank Corporation": "CSFL",
  "CoStar Group Inc.": "CSGP",
  "CSG Systems International Inc.": "CSGS",
  "Cardiovascular Systems Inc.": "CSII",
  "Canadian Solar Inc.": "CSIQ",
  "IQ Chaikin U.S. Small Cap ETF": "CSML",
  "Cornerstone OnDemand Inc.": "CSOD",
  "CSP Inc.": "CSPI",
  "Calamos Strategic Total Return Fund": "CSQ",
  "Chicken Soup for the Soul Entertainment Inc.": "CSSE",
  "Caesarstone Ltd.": "CSTE",
  "CapStar Financial Holdings Inc.": "CSTR",
  "Capital Southwest Corporation": "CSWC",
  "Capital Southwest Corporation 5.95% Notes due 2022": "CSWCL",
  "CSW Industrials Inc.": "CSWI",
  "Cintas Corporation": "CTAS",
  "Community Trust Bancorp Inc.": "CTBI",
  "Computer Task Group Incorporated": "CTG",
  "Charles & Colvard Ltd.": "CTHR",
  "CTI Industries Corporation": "CTIB",
  "CTI BioPharma Corp.": "CTIC",
  "CytomX Therapeutics Inc.": "CTMX",
  "CareTrust REIT Inc.": "CTRE",
  "Control4 Corporation": "CTRL",
  "Citi Trends Inc.": "CTRN",
  "Ctrip.com International Ltd.": "CTRP",
  "ContraVir Pharmaceuticals Inc": "CTRV",
  "Cognizant Technology Solutions Corporation": "CTSH",
  "CytoSorbents Corporation": "CTSO",
  "Connecticut Water Service Inc.": "CTWS",
  "Citius Pharmaceuticals Inc.": "CTXR",
  "Citius Pharmaceuticals Inc. Warrant": "CTXRW",
  "Citrix Systems Inc.": "CTXS",
  "The Herzfeld Caribbean Basin Fund Inc.": "CUBA",
  "Customers Bancorp Inc": "CUBI",
  "Customers Bancorp Inc 6.375% Senior Notes due 2018": "CUBS",
  "Cue Biopharma Inc.": "CUE",
  "CUI Global Inc.": "CUI",
  "Cutera Inc.": "CUTR",
  "CVB Financial Corporation": "CVBF",
  "Cavco Industries Inc.": "CVCO",
  "Central Valley Community Bancorp": "CVCY",
  "Commercial Vehicle Group Inc.": "CVGI",
  "Calavo Growers Inc.": "CVGW",
  "Commvault Systems Inc.": "CVLT",
  "Codorus Valley Bancorp Inc": "CVLY",
  "Covenant Transportation Group Inc.": "CVTI",
  "CVD Equipment Corporation": "CVV",
  "Coastway Bancorp Inc.": "CWAY",
  "Community West Bancshares": "CWBC",
  "CohBar Inc.": "CWBR",
  "Consolidated Water Co. Ltd.": "CWCO",
  "Casella Waste Systems Inc.": "CWST",
  "China XD Plastics Company Limited": "CXDC",
  "Concordia International Corp.": "CXRX",
  "WisdomTree China ex-State-Owned Enterprises Fund": "CXSE",
  "Cypress Semiconductor Corporation": "CY",
  "Celyad SA": "CYAD",
  "Cyanotech Corporation": "CYAN",
  "CyberOptics Corporation": "CYBE",
  "CyberArk Software Ltd.": "CYBR",
  "Cyclacel Pharmaceuticals Inc.": "CYCC",
  "Cyclacel Pharmaceuticals Inc. 6% Convertible Preferred Stock": "CYCCP",
  "Community Health Systems Inc. Series A Contingent Value Rights": "CYHHZ",
  "Changyou.com Limited": "CYOU",
  "CYREN Ltd.": "CYRN",
  "CryoPort Inc.": "CYRX",
  "CryoPort Inc. Warrants": "CYRXW",
  "Cytokinetics Incorporated": "CYTK",
  "CytRx Corporation": "CYTR",
  "Cytori Therapeutics Inc": "CYTX",
  "Cytori Therapeutics Inc Warrant": "CYTXW",
  "Citizens First Corporation": "CZFC",
  "Citizens & Northern Corp": "CZNC",
  "Caesars Entertainment Corporation": "CZR",
  "Citizens Community Bancorp Inc.": "CZWI",
  "Data I/O Corporation": "DAIO",
  "Daktronics Inc.": "DAKT",
  "Dare Bioscience Inc.": "DARE",
  "Famous Dave's of America Inc.": "DAVE",
  "Horizons DAX Germany ETF": "DAX",
  "DBV Technologies S.A.": "DBVT",
  "Diana Containerships Inc.": "DCIX",
  "Dime Community Bancshares Inc.": "DCOM",
  "Deciphera Pharmaceuticals Inc.": "DCPH",
  "Legg Mason Developed EX-US Diversified Core ETF": "DDBI",
  "Delta Technology Holdings Limited": "DELT",
  "Delta Technology Holdings Limited Warrant": "DELTW",
  "Denny's Corporation": "DENN",
  "Depomed Inc.": "DEPO",
  "Dermira Inc.": "DERM",
  "Destination Maternity Corporation": "DEST",
  "Differential Brands Group Inc.": "DFBG",
  "Diffusion Pharmaceuticals Inc.": "DFFN",
  "Davis Select Financial ETF": "DFNL",
  "Del Frisco's Restaurant Group Inc.": "DFRG",
  "Donegal Group Inc. Class A Common Stock": "DGICA",
  "Donegal Group Inc. Class B Common Stock": "DGICB",
  "Digi International Inc.": "DGII",
  "VelocityShares 3x Inverse Gold ETN": "DGLD",
  "Digital Ally Inc.": "DGLY",
  "WisdomTree Emerging Markets Quality Dividend Growth Fund": "DGRE",
  "WisdomTree U.S. SmallCap Quality Dividend Growth Fund": "DGRS",
  "WisdomTree U.S. Quality Dividend Growth Fund": "DGRW",
  "Diamond Hill Investment Group Inc.": "DHIL",
  "DHX Media Ltd.": "DHXM",
  "Diodes Incorporated": "DIOD",
  "Discovery Communications Inc. Series A Common Stock": "DISCA",
  "Discovery Communications Inc. Series B Common Stock": "DISCB",
  "Discovery Communications Inc.": "DISCK",
  "DISH Network Corporation": "DISH",
  "Daily Journal Corp. (S.C.)": "DJCO",
  "DLH Holdings Corp.": "DLHC",
  "Dynagas LNG Partners LP": "DLNG",
  "Dolphin Entertainment Inc.": "DLPN",
  "Dolphin Entertainment Inc. Warrant": "DLPNW",
  "Duluth Holdings Inc.": "DLTH",
  "Dollar Tree Inc.": "DLTR",
  "Dorchester Minerals L.P.": "DMLP",
  "DelMar Pharmaceuticals Inc.": "DMPI",
  "Digimarc Corporation": "DMRC",
  "DNB Financial Corp": "DNBF",
  "Dunkin' Brands Group Inc.": "DNKN",
  "Denali Therapeutics Inc.": "DNLI",
  "Dogness (International) Corporation": "DOGZ",
  "Dorman Products Inc.": "DORM",
  "Draper Oakwood Technology Acquisition Inc.": "DOTA",
  "Draper Oakwood Technology Acquisition Inc. Right": "DOTAR",
  "Draper Oakwood Technology Acquisition Inc. Unit": "DOTAU",
  "Draper Oakwood Technology Acquisition Inc. Warrant": "DOTAW",
  "Dova Pharmaceuticals Inc.": "DOVA",
  "Amdocs Limited": "DOX",
  "Digirad Corporation": "DRAD",
  "DarioHealth Corp.": "DRIO",
  "DarioHealth Corp. Warrant": "DRIOW",
  "Dicerna Pharmaceuticals Inc.": "DRNA",
  "DURECT Corporation": "DRRX",
  "DryShips Inc.": "DRYS",
  "The Descartes Systems Group Inc.": "DSGX",
  "Daseke Inc.": "DSKE",
  "Daseke Inc. Warrant": "DSKEW",
  "VelocityShares 3x Inverse Silver ETN": "DSLV",
  "DSP Group Inc.": "DSPG",
  "Deswell Industries Inc.": "DSWL",
  "DAVIDsTEA Inc.": "DTEA",
  "Determine Inc.": "DTRM",
  "DTE Energy Company 2016 Corporate Units": "DTV",
  "Davis Select U.S. Equity ETF": "DUSA",
  "Dynavax Technologies Corporation": "DVAX",
  "Diversicare Healthcare Services Inc.": "DVCR",
  "Elkhorn Commodity Rotation Strategy ETF": "DWAC",
  "Arrow DWA Tactical ETF": "DWAT",
  "Datawatch Corporation": "DWCH",
  "Arrow DWA Country Rotation ETF": "DWCR",
  "SPDR Dorsey Wright Fixed Income Allocation ETF": "DWFI",
  "PowerShares DWA Tactical Multi-Asset Income Portfolio": "DWIN",
  "Davis Select Worldwide ETF": "DWLD",
  "PowerShares DWA Momentum & Low Volatility Rotation Portfolio": "DWLV",
  "Dawson Geophysical Company": "DWSN",
  "PowerShares DWA Tactical Sector Rotation Portfolio": "DWTR",
  "DexCom Inc.": "DXCM",
  "WisdomTree Germany Hedged Equity Fund": "DXGE",
  "WisdomTree Japan Hedged SmallCap Equity Fund": "DXJS",
  "Destination XL Group Inc.": "DXLG",
  "DXP Enterprises Inc.": "DXPE",
  "WisdomTree United Kingdom Hedged Equity Fund": "DXPS",
  "Dextera Surgical Inc.": "DXTR",
  "The Dixie Group Inc.": "DXYN",
  "Dynatronics Corporation": "DYNT",
  "Dynasil Corporation of America": "DYSL",
  "DASAN Zhone Solutions Inc.": "DZSI",
  "Electronic Arts Inc.": "EA",
  "Easterly Acquisition Corp.": "EACQ",
  "Easterly Acquisition Corp. Unit": "EACQU",
  "Easterly Acquisition Corp. Warrant": "EACQW",
  "Platinum Eagle Acquisition Corp. Unit": "EAGLU",
  "Auris Medical Holding AG": "EARS",
  "Eastside Distilling Inc.": "EAST",
  "Eastside Distilling Inc. Warrants": "EASTW",
  "eBay Inc.": "EBAY",
  "eBay Inc. 6.0% Notes Due 2056": "EBAYL",
  "Eleven Biotherapeutics Inc.": "EBIO",
  "Ebix Inc.": "EBIX",
  "Eagle Bancorp Montana Inc.": "EBMT",
  "Meridian Bancorp Inc.": "EBSB",
  "Enterprise Bancorp Inc": "EBTC",
  "Echo Global Logistics Inc.": "ECHO",
  "US Ecology Inc.": "ECOL",
  "Encore Capital Group Inc": "ECPG",
  "Endocyte Inc.": "ECYT",
  "EDAP TMS S.A.": "EDAP",
  "Legg Mason Emerging Markets Diversified Core ETF": "EDBI",
  "Edge Therapeutics Inc.": "EDGE",
  "Edgewater Technology Inc.": "EDGW",
  "Editas Medicine Inc.": "EDIT",
  "Educational Development Corporation": "EDUC",
  "Euronet Worldwide Inc.": "EEFT",
  "Ecology and Environment Inc.": "EEI",
  "iShares MSCI Emerging Markets Asia Index Fund": "EEMA",
  "Global X MSCI SuperDividend EAFE ETF": "EFAS",
  "Eagle Financial Bancorp Inc.": "EFBI",
  "Electronics for Imaging Inc.": "EFII",
  "Energy Focus Inc.": "EFOI",
  "Enterprise Financial Services Corporation": "EFSC",
  "eGain Corporation": "EGAN",
  "Eagle Bancorp Inc.": "EGBN",
  "8x8 Inc": "EGHT",
  "Eagle Bulk Shipping Inc.": "EGLE",
  "Egalet Corporation": "EGLT",
  "NIC Inc.": "EGOV",
  "Eagle Pharmaceuticals Inc.": "EGRX",
  "eHealth Inc.": "EHTH",
  "Endurance International Group Holdings Inc.": "EIGI",
  "Eiger BioPharmaceuticals Inc.": "EIGR",
  "Ekso Bionics Holdings Inc.": "EKSO",
  "Electrum Special Acquisition Corporation": "ELEC",
  "Electrum Special Acquisition Corporation Unit": "ELECU",
  "Electrum Special Acquisition Corporation Warrants": "ELECW",
  "Endologix Inc.": "ELGX",
  "Echelon Corporation": "ELON",
  "Electro-Sensors Inc.": "ELSE",
  "Eltek Ltd.": "ELTK",
  "WisdomTree Emerging Markets Corporate Bond Fund": "EMCB",
  "Emclaire Financial Corp": "EMCF",
  "WisdomTree Emerging Markets Consumer Growth Fund": "EMCG",
  "EMC Insurance Group Inc.": "EMCI",
  "iShares S&P Emerging Markets Infrastructure Index Fund": "EMIF",
  "Elbit Imaging Ltd.": "EMITF",
  "EMCORE Corporation": "EMKR",
  "Eastern Company (The)": "EML",
  "Emmis Communications Corporation": "EMMS",
  "iShares MSCI Emerging Markets ex China ETF": "EMXC",
  "Endo International plc": "ENDP",
  "Entegra Financial Corp.": "ENFC",
  "ENGlobal Corporation": "ENG",
  "Enphase Energy Inc.": "ENPH",
  "The Ensign Group Inc.": "ENSG",
  "Global Eagle Entertainment Inc.": "ENT",
  "Enanta Pharmaceuticals Inc.": "ENTA",
  "Entegris Inc.": "ENTG",
  "Entellus Medical Inc.": "ENTL",
  "ERShares Entrepreneur 30": "ENTR",
  "Enzymotec Ltd.": "ENZY",
  "Bottomline Technologies Inc.": "EPAY",
  "ESSA Pharma Inc.": "EPIX",
  "Epizyme Inc.": "EPZM",
  "Equity Bancshares Inc.": "EQBK",
  "Equitable Financial Corp.": "EQFN",
  "Equinix Inc.": "EQIX",
  "ProShares Equities for Rising Rates ETF": "EQRR",
  "Eldorado Resorts Inc.": "ERI",
  Ericsson: "ERIC",
  "Erie Indemnity Company": "ERIE",
  "Energy Recovery Inc.": "ERII",
  "Erytech Pharma S.A.": "ERYP",
  "Elmira Savings Bank NY (The)": "ESBK",
  "Escalade Incorporated": "ESCA",
  "Euroseas Ltd.": "ESEA",
  "Eco-Stim Energy Solutions Inc.": "ESES",
  "FlexShares STOXX US ESG Impact Index Fund": "ESG",
  "iShares MSCI EAFE ESG Optimized ETF": "ESGD",
  "iShares MSCI EM ESG Optimized ETF": "ESGE",
  "FlexShares STOXX Global ESG Impact Index Fund": "ESGG",
  "Enstar Group Limited": "ESGR",
  "iShares MSCI USA ESG Optimized ETF": "ESGU",
  "Electro Scientific Industries Inc.": "ESIO",
  "Elbit Systems Ltd.": "ESLT",
  "Essendant Inc.": "ESND",
  "Esperion Therapeutics Inc.": "ESPR",
  "Esquire Financial Holdings Inc.": "ESQ",
  "Express Scripts Holding Company": "ESRX",
  "ESSA Bancorp Inc.": "ESSA",
  "Estre Ambiental Inc.": "ESTR",
  "Estre Ambiental Inc. Warrants": "ESTRW",
  "Community Bankers Trust Corporation.": "ESXB",
  "E*TRADE Financial Corporation": "ETFC",
  "Etsy Inc.": "ETSY",
  "iShares MSCI Europe Financials Sector Index Fund": "EUFN",
  "Everbridge Inc.": "EVBG",
  "EV Energy Partners L.P.": "EVEP",
  "Evofem Biosciences Inc.": "EVFM",
  "Eaton Vance Floating-Rate NextShares": "EVFTCNX",
  "Eaton Vance Global Income Builder NextShares": "EVGBCNX",
  "Eaton Vance NextShares Trust II": "EVLMC",
  "Eaton Vance TABS 5-to-15 Year Laddered Municipal Bond NextShares": "EVLMCNX",
  "EVINE Live Inc.": "EVLV",
  "Evoke Pharma Inc.": "EVOK",
  "Evolving Systems Inc.": "EVOL",
  "Eaton Vance NextShares Trust": "EVSTC",
  "Eaton Vance Stock NextShares": "EVSTCNX",
  "East West Bancorp Inc.": "EWBC",
  "Exactech Inc.": "EXAC",
  "Exact Sciences Corporation": "EXAS",
  "Exelixis Inc.": "EXEL",
  "EXFO Inc": "EXFO",
  "ExlService Holdings Inc.": "EXLS",
  "Expeditors International of Washington Inc.": "EXPD",
  "Expedia Inc.": "EXPE",
  "Exponent Inc.": "EXPO",
  "Extreme Networks Inc.": "EXTR",
  "Energy XXI Gulf Coast Inc.": "EXXI",
  "National Vision Holdings Inc.": "EYE",
  "Eyegate Pharmaceuticals Inc.": "EYEG",
  "Eyegate Pharmaceuticals Inc. Warrants": "EYEGW",
  "Second Sight Medical Products Inc.": "EYES",
  "Second Sight Medical Products Inc. Warrants": "EYESW",
  "EZCORP Inc.": "EZPW",
  "First Trust Alternative Absolute Return Strategy ETF": "FAAR",
  "iShares Fallen Angels USD Bond ETF": "FALN",
  "Diamondback Energy Inc.": "FANG",
  "Fanhua Inc.": "FANH",
  "Farmer Brothers Company": "FARM",
  "FARO Technologies Inc.": "FARO",
  "Fastenal Company": "FAST",
  "FAT Brands Inc.": "FAT",
  "Fate Therapeutics Inc.": "FATE",
  "Facebook Inc.": "FB",
  "Fortress Biotech Inc.": "FBIO",
  "Fortress Biotech Inc. 9.375% Series A Cumulative Redeemable Perpetual Preferred Stock": "FBIOP",
  "First Business Financial Services Inc.": "FBIZ",
  "The First Bancshares Inc.": "FBMS",
  "First Bancorp": "FBNC",
  "First Connecticut Bancorp Inc.": "FBNK",
  "Fauquier Bankshares Inc.": "FBSS",
  "First Trust California Municipal High income ETF": "FCAL",
  "First Capital Inc.": "FCAP",
  "First Community Bancshares Inc.": "FCBC",
  "First Community Corporation": "FCCO",
  "1st Constitution Bancorp (NJ)": "FCCY",
  "First Trust CEF Income Opportunity ETF": "FCEF",
  "FuelCell Energy Inc.": "FCEL",
  "First Cash Inc.": "FCFS",
  "First Citizens BancShares Inc. Class A Common Stock": "FCNCA",
  "FC Global Realty Incorporated": "FCRE",
  "Fibrocell Science Inc": "FCSC",
  "First Trust SSI Strategic Convertible Securities ETF": "FCVT",
  "Fidelity D & D Bancorp Inc.": "FDBC",
  "First Defiance Financial Corp.": "FDEF",
  "First Trust Strategic Income ETF": "FDIV",
  "Fidus Investment Corporation": "FDUS",
  "Frequency Electronics Inc.": "FEIM",
  "Franklin Electric Co. Inc.": "FELE",
  "First Trust Emerging Markets Local Currency Bond ETF": "FEMB",
  "Fennec Pharmaceuticals Inc.": "FENC",
  "First Trust Eurozone AlphaDEX ETF": "FEUZ",
  "FireEye Inc.": "FEYE",
  "First Financial Bancorp.": "FFBC",
  "First Financial Bancorp. Warrant 12/23/2018": "FFBCW",
  "FFBW Inc.": "FFBW",
  "Fuwei Films (Holdings) Co. Ltd.": "FFHL",
  "Flushing Financial Corporation": "FFIC",
  "First Financial Bankshares Inc.": "FFIN",
  "F5 Networks Inc.": "FFIV",
  "Farmers Capital Bank Corporation": "FFKT",
  "First Financial Northwest Inc.": "FFNW",
  "First Foundation Inc.": "FFWM",
  "FGL Holdings": "FG",
  "CF Corporation Warrants": "FG.W",
  "First Guaranty Bancshares Inc.": "FGBI",
  "FibroGen Inc": "FGEN",
  "FORM Holdings Corp.": "FH",
  "First Hawaiian Inc.": "FHB",
  "First Interstate BancSystem Inc.": "FIBK",
  "The Finish Line Inc.": "FINL",
  "Global X FinTech ETF": "FINX",
  "Financial Institutions Inc.": "FISI",
  "Fiserv Inc.": "FISV",
  "Fifth Third Bancorp": "FITB",
  "Fifth Third Bancorp Depositary Share repstg 1/1000th Ownership Interest Perp Pfd Series I": "FITBI",
  "Five Below Inc.": "FIVE",
  "Five9 Inc.": "FIVN",
  "First Trust TCW Opportunistic Fixed Income ETF": "FIXD",
  "National Beverage Corp.": "FIZZ",
  "Fluidigm Corporation": "FLDM",
  "Flex Ltd.": "FLEX",
  "Fulgent Genetics Inc.": "FLGT",
  "The First of Long Island Corporation": "FLIC",
  "FLIR Systems Inc.": "FLIR",
  "Flex Pharma Inc.": "FLKS",
  "Full House Resorts Inc.": "FLL",
  "SPX FLOW Inc.": "FLOW",
  "1-800 FLOWERS.COM Inc.": "FLWS",
  "Flexion Therapeutics Inc.": "FLXN",
  "Flexsteel Industries Inc.": "FLXS",
  "Farmers & Merchants Bancorp Inc.": "FMAO",
  "First Trust Managed Municipal ETF": "FMB",
  "First Mid-Illinois Bancshares Inc.": "FMBH",
  "First Midwest Bancorp Inc.": "FMBI",
  "Forum Merger Corporation": "FMCI",
  "Forum Merger Corporation Right": "FMCIR",
  "Forum Merger Corporation Unit": "FMCIU",
  "Forum Merger Corporation Warrant": "FMCIW",
  "First Trust Municipal High Income ETF": "FMHI",
  "Foundation Medicine Inc.": "FMI",
  "Farmers National Banc Corp.": "FMNB",
  "FNB Bancorp": "FNBG",
  "Financial Engines Inc.": "FNGN",
  "Federated National Holding Company": "FNHC",
  "Finjan Holdings Inc.": "FNJN",
  "Funko Inc.": "FNKO",
  "First Bancorp Inc (ME)": "FNLC",
  "Finisar Corporation": "FNSR",
  "FinTech Acquisition Corp. II": "FNTE",
  "FinTech Acquisition Corp. II Unit": "FNTEU",
  "FinTech Acquisition Corp. II Warrant": "FNTEW",
  "First Northwest Bancorp": "FNWB",
  "Gabelli NextShares Trust": "FOANC",
  "Gabelli Food of All Nations NextShares": "FOANCNX",
  "Fogo de Chao Inc.": "FOGO",
  "Amicus Therapeutics Inc.": "FOLD",
  "Foamix Pharmaceuticals Ltd.": "FOMX",
  "First Trust NASDAQ Smartphone Index Fund": "FONE",
  "Fonar Corporation": "FONR",
  "Forward Industries Inc.": "FORD",
  "Fuling Global Inc.": "FORK",
  "FormFactor Inc.": "FORM",
  "Forrester Research Inc.": "FORR",
  "Formula Systems (1985) Ltd. ADS represents 1 ordinary shares": "FORTY",
  "Fossil Group Inc.": "FOSL",
  "Twenty-First Century Fox Inc.": "FOX",
  "Fox Factory Holding Corp.": "FOXF",
  "FlexShopper Inc.": "FPAY",
  "Five Prime Therapeutics Inc.": "FPRX",
  "First Trust International IPO ETF": "FPXI",
  "Francesca's Holdings Corporation": "FRAN",
  "First Bank": "FRBA",
  "Republic First Bancorp Inc.": "FRBK",
  "Fred's Inc.": "FRED",
  "Fiesta Restaurant Group Inc.": "FRGI",
  "First Merchants Corporation": "FRME",
  "FRP Holdings Inc.": "FRPH",
  "Freshpet Inc.": "FRPT",
  "Papa Murphy's Holdings Inc.": "FRSH",
  "Foresight Autonomous Holdings Ltd.": "FRSX",
  "Forterra Inc.": "FRTA",
  "Federal Street Acquisition Corp.": "FSAC",
  "Federal Street Acquisition Corp. Unit consisting of One Common Stock and Half of a Warrant": "FSACU",
  "Federal Street Acquisition Corp. Warrant": "FSACW",
  "FSB Bancorp Inc.": "FSBC",
  "FS Bancorp Inc.": "FSBW",
  "ForeScout Technologies Inc.": "FSCT",
  "First Savings Financial Group Inc.": "FSFG",
  "First Solar Inc.": "FSLR",
  "Fusion Telecommunications International Inc.": "FSNN",
  "L.B. Foster Company": "FSTR",
  "FirstService Corporation": "FSV",
  "First Trust Indxx Global Agriculture ETF": "FTAG",
  "First Trust Capital Strength ETF": "FTCS",
  "FTD Companies Inc.": "FTD",
  "Fuel Tech Inc.": "FTEK",
  "FRONTEO Inc.": "FTEO",
  "Future FinTech Group Inc.": "FTFT",
  "First Trust Global Tactical Commodity Strategy Fund": "FTGC",
  "First Trust BuyWrite Income ETF": "FTHI",
  "First Trust Hedged BuyWrite Income ETF": "FTLB",
  "Fortinet Inc.": "FTNT",
  "Frontier Communications Corporation": "FTR",
  "First Trust Indxx Global Natural Resources Income ETF": "FTRI",
  "Frontier Communications Corporation 11.125% Series A Mandatory Convertible Preferred Stock": "FTRPR",
  "First Trust Senior Loan Fund ETF": "FTSL",
  "First Trust Enhanced Short Maturity ETF": "FTSM",
  "First Trust Nasdaq Retail ETF": "FTXD",
  "First Trust Nasdaq Food & Beverage ETF": "FTXG",
  "First Trust Nasdaq Pharmaceuticals ETF": "FTXH",
  "First Trust Nasdaq Semiconductor ETF": "FTXL",
  "First Trust Nasdaq Oil & Gas ETF": "FTXN",
  "First Trust Nasdaq Bank ETF": "FTXO",
  "First Trust Nasdaq Transportation ETF": "FTXR",
  "Fulton Financial Corporation": "FULT",
  "First United Corporation": "FUNC",
  "Sprott Focus Trust Inc.": "FUND",
  "First US Bancshares Inc.": "FUSB",
  "Arcimoto Inc.": "FUV",
  "First Trust Dorsey Wright Focus 5 ETF": "FV",
  "First Trust Dorsey Wright Dynamic Focus 5 ETF": "FVC",
  "Liberty Media Corporation Series A": "FWONA",
  "Liberty Media Corporation Series C": "FWONK",
  "Forward Pharma A/S": "FWP",
  "Forward Air Corporation": "FWRD",
  "German American Bancorp Inc.": "GABC",
  "Gaia Inc.": "GAIA",
  "Gladstone Investment Corporation": "GAIN",
  "Gladstone Investment Corporation 6.25% Series D Cumulative Term Preferred Stock": "GAINM",
  "Gladstone Investment Corporation 6.50% Series C Cumulative Term Preferred Stock Due 2022": "GAINN",
  "Gladstone Investment Corporation 6.75% Series B Cumulative Term Preferred Stock": "GAINO",
  "Galectin Therapeutics Inc.": "GALT",
  "Garrison Capital Inc.": "GARS",
  "StealthGas Inc.": "GASS",
  "Glacier Bancorp Inc.": "GBCI",
  "Golub Capital BDC Inc.": "GBDC",
  "Global Indemnity Limited": "GBLI",
  "Global Indemnity Limited 7.875% Subordinated Notes due 2047": "GBLIL",
  "Global Indemnity Limited 7.75% Subordinated Note Due 2045": "GBLIZ",
  "Guaranty Bancorp": "GBNK",
  "Global Blood Therapeutics Inc.": "GBT",
  "Greene County Bancorp Inc.": "GCBC",
  "Sanofi Contingent Value Right (Expiring 12/31/2020)": "GCVRZ",
  "Golden Entertainment Inc.": "GDEN",
  "GDS Holdings Limited": "GDS",
  "Great Elm Capital Group Inc.": "GEC",
  "Great Elm Capital Corp.": "GECC",
  "Great Elm Capital Corp. 6.50% Notes due 2022": "GECCL",
  "Gemphire Therapeutics Inc.": "GEMP",
  "Gencor Industries Inc.": "GENC",
  "Genetic Technologies Ltd": "GENE",
  "Principal Millennials Index ETF": "GENY",
  "Geospace Technologies Corporation": "GEOS",
  "Geron Corporation": "GERN",
  "Gevo Inc.": "GEVO",
  "Guaranty Federal Bancshares Inc.": "GFED",
  "General Finance Corporation": "GFN",
  "General Finance Corporation Cumulative Redeemable Perpetual Preferred Series C": "GFNCP",
  "General Finance Corporation Senior Notes due 2021": "GFNSL",
  "Grupo Financiero Galicia S.A.": "GGAL",
  "Genomic Health Inc.": "GHDX",
  "Gulf Island Fabrication Inc.": "GIFI",
  "GigaMedia Limited": "GIGM",
  "G-III Apparel Group LTD.": "GIII",
  "Gilead Sciences Inc.": "GILD",
  "Gilat Satellite Networks Ltd.": "GILT",
  "Gladstone Capital Corporation": "GLAD",
  "Gladstone Capital Corporation 6.00% Series 2024 Term Preferred Stock": "GLADN",
  "TerraForm Global Inc.": "GLBL",
  "Globus Maritime Limited": "GLBS",
  "Glen Burnie Bancorp": "GLBZ",
  "Great Lakes Dredge & Dock Corporation": "GLDD",
  "Credit Suisse Gold Shares Covered Call Exchange Traded Notes": "GLDI",
  "Galmed Pharmaceuticals Ltd.": "GLMD",
  "Golar LNG Limited": "GLNG",
  "Galapagos NV": "GLPG",
  "Gaming and Leisure Properties Inc.": "GLPI",
  "Greenlight Reinsurance Ltd.": "GLRE",
  "Glu Mobile Inc.": "GLUU",
  "GlycoMimetics Inc.": "GLYC",
  "Golar LNG Partners LP": "GMLP",
  "Golar LNG Partners LP 8.75% Series A Cumulative Redeemable Preferred Units": "GMLPP",
  "Green Bancorp Inc.": "GNBC",
  "Genocea Biosciences Inc.": "GNCA",
  "General Communication Inc. Class A Common Stock": "GNCMA",
  "iShares GNMA Bond ETF": "GNMA",
  "GenMark Diagnostics Inc.": "GNMK",
  "VanEck Vectors Generic Drugs ETF": "GNRX",
  "Gentex Corporation": "GNTX",
  "Guaranty Bancshares Inc.": "GNTY",
  "Genius Brands International Inc.": "GNUS",
  "Golden Ocean Group Limited": "GOGL",
  "Gogo Inc.": "GOGO",
  "Randgold Resources Limited": "GOLD",
  "Acushnet Holdings Corp.": "GOLF",
  "Gladstone Commercial Corporation": "GOOD",
  "Gladstone Commercial Corporation Series D Cumulative Redeemable Preferred Stock": "GOODM",
  "Gladstone Commercial Corporation 7.50% Series B Cumulative Redeemable Preferred Stock": "GOODO",
  "Gladstone Commercial Corporation 7.75% Series A Cumulative Redeemable Preferred Stock": "GOODP",
  "Alphabet Inc.": "GOOGL",
  "Global Partner Acquisition Corp.": "GPAC",
  "Global Partner Acquisition Corp. Unit": "GPACU",
  "Global Partner Acquisition Corp. Warrants": "GPACW",
  "Gaming Partners International Corporation": "GPIC",
  "Gulfport Energy Corporation": "GPOR",
  "Green Plains Partners LP": "GPP",
  "Green Plains Inc.": "GPRE",
  "GoPro Inc.": "GPRO",
  "Green Brick Partners Inc.": "GRBK",
  "Grifols S.A.": "GRFS",
  "First Trust NASDAQ Clean Edge Smart Grid Infrastructure Index Fund": "GRID",
  "Griffin Industrial Realty Inc.": "GRIF",
  "Garmin Ltd.": "GRMN",
  "VanEck Vectors Green Bond": "GRNB",
  "U.S. Global Investors Inc.": "GROW",
  "Groupon Inc.": "GRPN",
  "GRAVITY Co. Ltd.": "GRVY",
  "Great Southern Bancorp Inc.": "GSBC",
  "Gores Holdings II Inc.": "GSHT",
  "Gores Holdings II Inc. Unit": "GSHTU",
  "Gores Holdings II Inc. Warrants": "GSHTW",
  "GSI Technology Inc.": "GSIT",
  "Ferroglobe PLC": "GSM",
  "Gridsum Holding Inc.": "GSUM",
  "GSV Capital Corp": "GSVC",
  "The Goodyear Tire & Rubber Company": "GT",
  "G1 Therapeutics Inc.": "GTHX",
  "Good Times Restaurants Inc.": "GTIM",
  "Chart Industries Inc.": "GTLS",
  "GTx Inc.": "GTXI",
  "GTY Technology Holdings Inc.": "GTYH",
  "GTY Technology Holdings Inc. Unit": "GTYHU",
  "GTY Technology Holdings Inc. Warrants": "GTYHW",
  "WisdomTree Middle East Dividend Fund": "GULF",
  "Gulf Resources Inc.": "GURE",
  "GWG Holdings Inc": "GWGH",
  "GW Pharmaceuticals Plc": "GWPH",
  "Global Water Resources Inc.": "GWRS",
  "Gyrodyne  LLC": "GYRO",
  "Hawaiian Holdings Inc.": "HA",
  "The Habit Restaurants Inc.": "HABT",
  "Hanmi Financial Corporation": "HAFC",
  "The Hain Celestial Group Inc.": "HAIN",
  "Restoration Robotics Inc.": "HAIR",
  "Hallmark Financial Services Inc.": "HALL",
  "Halozyme Therapeutics Inc.": "HALO",
  "Hasbro Inc.": "HAS",
  "Blackhawk Network Holdings Inc.": "HAWK",
  "Haynes International Inc.": "HAYN",
  "Huntington Bancshares Incorporated": "HBAN",
  "Huntington Bancshares Incorporated Depositary Shares each representing a 1/40th interest in a share of 5.875% Series C Non-Cumulative Perpetual Preferred Stock":
    "HBANN",
  "Huntington Bancshares Incorporated Depositary Shares": "HBANO",
  "Huntington Bancshares Incorporated Non Cumulative Perp Conv Pfd Ser A": "HBANP",
  "Home Bancorp Inc.": "HBCP",
  "Hancock Holding Company": "HBHC",
  "Hancock Holding Company 5.95% Subordinated Notes due 2045": "HBHCL",
  "Harvard Bioscience Inc.": "HBIO",
  "Hamilton Bancorp Inc.": "HBK",
  "Howard Bancorp Inc.": "HBMD",
  "Horizon Bancorp (IN)": "HBNC",
  "Huttig Building Products Inc.": "HBP",
  "Harvest Capital Credit Corporation": "HCAP",
  "Harvest Capital Credit Corporation 6.125% Notes due 2022": "HCAPZ",
  "Heritage-Crystal Clean Inc.": "HCCI",
  "The Hackett Group Inc.": "HCKT",
  "Hutchison China MediTech Limited": "HCM",
  "Hawaiian Telcom Holdco Inc.": "HCOM",
  "Healthcare Services Group Inc.": "HCSG",
  "Hardinge Inc.": "HDNG",
  "Hortonworks Inc.": "HDP",
  "HD Supply Holdings Inc.": "HDS",
  "Hudson Technologies Inc.": "HDSN",
  "Turtle Beach Corporation": "HEAR",
  "Hebron Technology Co. Ltd.": "HEBT",
  "H&E Equipment Services Inc.": "HEES",
  "Helen of Troy Limited": "HELE",
  "HopFed Bancorp Inc.": "HFBC",
  "Home Federal Bancorp Inc. of Louisiana": "HFBL",
  "Hartford Funds NextShares Trust": "HFGIC",
  "Hartford Global Impact NextShares Fund": "HFGICNX",
  "Heritage Financial Corporation": "HFWA",
  "China HGS Real Estate Inc.": "HGSH",
  "Hibbett Sports Inc.": "HIBB",
  "Hingham Institution for Savings": "HIFS",
  "Highway Holdings Limited": "HIHO",
  "Health Insurance Innovations Inc.": "HIIQ",
  "Himax Technologies Inc.": "HIMX",
  "Hailiang Education Group Inc.": "HLG",
  "Harmonic Inc.": "HLIT",
  "Hamilton Lane Incorporated": "HLNE",
  "Houghton Mifflin Harcourt Company": "HMHC",
  "HMN Financial Inc.": "HMNF",
  "Helios and Matheson Analytics Inc": "HMNY",
  "HomeStreet Inc.": "HMST",
  "HMS Holdings Corp": "HMSY",
  "HomeTown Bankshares Corporation": "HMTA",
  "Hemisphere Media Group Inc.": "HMTV",
  "Strategy Shares Nasdaq 7HANDL Index ETF": "HNDL",
  "Hennessy Advisors Inc.": "HNNA",
  "Hallador Energy Company": "HNRG",
  "Hooker Furniture Corporation": "HOFT",
  "Hollysys Automation Technologies Ltd.": "HOLI",
  "Hologic Inc.": "HOLX",
  "Home BancShares Inc.": "HOMB",
  "At Home Group Inc.": "HOME",
  "HarborOne Bancorp Inc.": "HONE",
  "Hope Bancorp Inc.": "HOPE",
  "Hovnanian Enterprises Inc Depositary Share representing 1/1000th of 7.625% Series A Preferred Stock": "HOVNP",
  "Highpower International Inc": "HPJ",
  "Hanwha Q CELLS Co. Ltd.": "HQCL",
  "HealthEquity Inc.": "HQY",
  "Heron Therapeutics Inc.": "HRTX",
  "Horizon Technology Finance Corporation": "HRZN",
  "Histogenics Corporation": "HSGX",
  "Henry Schein Inc.": "HSIC",
  "Heidrick & Struggles International Inc.": "HSII",
  "Heska Corporation": "HSKA",
  "HSN Inc.": "HSNI",
  "Hudson Global Inc.": "HSON",
  "HealthStream Inc.": "HSTM",
  "HomeTrust Bancshares Inc.": "HTBI",
  "Heritage Commerce Corp": "HTBK",
  "Heat Biologics Inc.": "HTBX",
  "HTG Molecular Diagnostics Inc.": "HTGM",
  "China Lodging Group Limited": "HTHT",
  "Heartland Express Inc.": "HTLD",
  "Heartland Financial USA Inc.": "HTLF",
  "Hub Group Inc.": "HUBG",
  "Hunter Maritime Acquisition Corp.": "HUNT",
  "Hunter Maritime Acquisition Corp. Unit": "HUNTU",
  "Hunter Maritime Acquisition Corp. Warrant": "HUNTW",
  "Hurco Companies Inc.": "HURC",
  "Huron Consulting Group Inc.": "HURN",
  "HV Bancorp Inc.": "HVBC",
  "Hawthorn Bancshares Inc.": "HWBK",
  "Houston Wire & Cable Company": "HWCC",
  "Hawkins Inc.": "HWKN",
  "Hexindai Inc.": "HX",
  "Haymaker Acquisition Corp.": "HYAC",
  "Haymaker Acquisition Corp. Unit": "HYACU",
  "Haymaker Acquisition Corp. Warrant": "HYACW",
  "Hydrogenics Corporation": "HYGS",
  "First Trust High Yield Long/Short ETF": "HYLS",
  "WisdomTree Negative Duration High Yield Bond Fund": "HYND",
  "iShares iBoxx $ High Yield ex Oil & Gas Corporate Bond ETF": "HYXE",
  "WisdomTree Interest Rate Hedged High Yield Bond Fund": "HYZD",
  "Horizon Pharma plc": "HZNP",
  "IAC/InterActiveCorp": "IAC",
  "I-AM Capital Acquisition Company": "IAM",
  "I-AM Capital Acquisition Company Right": "IAMXR",
  "I-AM Capital Acquisition Company Warrant": "IAMXW",
  "Integra LifeSciences Holdings Corporation": "IART",
  "iShares Nasdaq Biotechnology Index Fund": "IBB",
  "Independent Bank Corporation": "IBCP",
  "IBERIABANK Corporation": "IBKC",
  "IBERIABANK Corporation Depositary Shares Representing Series C Fixed to Floating": "IBKCO",
  "IBERIABANK Corporation Depositary Shares Representing Series B Fixed to Floating": "IBKCP",
  "Interactive Brokers Group Inc.": "IBKR",
  "International Bancshares Corporation": "IBOC",
  "Independent Bank Group Inc": "IBTX",
  "Amplify Online Retail ETF": "IBUY",
  "icad inc.": "ICAD",
  "County Bancorp Inc.": "ICBK",
  "ImmuCell Corporation": "ICCC",
  "ICC Holdings Inc.": "ICCH",
  "ICF International Inc.": "ICFI",
  "Ichor Holdings": "ICHR",
  "iClick Interactive Asia Group Limited": "ICLK",
  "iShares S&P Global Clean Energy Index Fund": "ICLN",
  "ICON plc": "ICLR",
  "Iconix Brand Group Inc.": "ICON",
  "Intercept Pharmaceuticals Inc.": "ICPT",
  "ICU Medical Inc.": "ICUI",
  "InterDigital Inc.": "IDCC",
  "PowerShares FTSE International Low Beta Equal Weight Portfolio": "IDLB",
  "Idera Pharmaceuticals Inc.": "IDRA",
  "Industrial Services of America Inc.": "IDSA",
  "I.D. Systems Inc.": "IDSY",
  "Integrated Device Technology Inc.": "IDTI",
  "Interpace Diagnostics Group Inc.": "IDXG",
  "IDEXX Laboratories Inc.": "IDXX",
  "Icahn Enterprises L.P.": "IEP",
  "IES Holdings Inc.": "IESC",
  "iShares MSCI Europe Small-Cap ETF": "IEUS",
  "iShares FTSE EPRA/NAREIT Europe Index Fund": "IFEU",
  "iShares FTSE EPRA/NAREIT Global Real Estate ex-U.S. Index Fund": "IFGL",
  "iFresh Inc.": "IFMK",
  "InfoSonics Corp": "IFON",
  "InflaRx N.V.": "IFRX",
  "First Trust Dorsey Wright International Focus 5 ETF": "IFV",
  "Internet Gold Golden Lines Ltd.": "IGLD",
  "iShares International Treasury Bond ETF": "IGOV",
  "Information Services Group Inc.": "III",
  "Insteel Industries Inc.": "IIIN",
  "Internet Initiative Japan Inc.": "IIJI",
  "IntriCon Corporation": "IIN",
  "II-VI Incorporated": "IIVI",
  "Ikonics Corporation": "IKNX",
  "ILG Inc": "ILG",
  "Illumina Inc.": "ILMN",
  "Industrial Logistics Properties Trust": "ILPT",
  "Immune Design Corp.": "IMDZ",
  "ImmunoGen Inc.": "IMGN",
  "Intermolecular Inc.": "IMI",
  "Ingles Markets Incorporated Class A Common Stock": "IMKTA",
  "Immutep Limited": "IMMP",
  "Immersion Corporation": "IMMR",
  "Immunomedics Inc.": "IMMU",
  "Imprimis Pharmaceuticals Inc.": "IMMY",
  "Immune Pharmaceuticals Inc.": "IMNP",
  "ChipMOS TECHNOLOGIES INC.": "IMOS",
  "Immuron Limited": "IMRN",
  "Immuron Limited Warrants": "IMRNW",
  "Integrated Media Technology Limited": "IMTE",
  "Internap Corporation": "INAP",
  "First Internet Bancorp": "INBK",
  "First Internet Bancorp 6.0% Fixed-to-Floating Rate Subordinated Notes due 2026": "INBKL",
  "INC Research Holdings Inc.": "INCR",
  "Incyte Corporation": "INCY",
  "Independent Bank Corp.": "INDB",
  "Industrea Acquisition Corp.": "INDU",
  "Industrea Acquisition Corp. Unit": "INDUU",
  "Industrea Acquisition Corp. Warrant": "INDUW",
  "iShares S&P India Nifty 50 Index Fund": "INDY",
  "Infinity Pharmaceuticals Inc.": "INFI",
  "Infinera Corporation": "INFN",
  "IHS Markit Ltd.": "INFO",
  "Legg Mason Global Infrastructure ETF": "INFR",
  "Inogen Inc": "INGN",
  "Inovio Pharmaceuticals Inc.": "INO",
  "Innodata Inc.": "INOD",
  "Inovalon Holdings Inc.": "INOV",
  Inpixon: "INPX",
  "Inspired Entertainment Inc.": "INSE",
  "Inseego Corp.": "INSG",
  "Insmed Inc.": "INSM",
  "Insys Therapeutics Inc.": "INSY",
  "Intel Corporation": "INTC",
  "The Intergroup Corporation": "INTG",
  "INTL FCStone Inc.": "INTL",
  "Intuit Inc.": "INTU",
  "Intersections Inc.": "INTX",
  "Innoviva Inc.": "INVA",
  "Identiv Inc.": "INVE",
  "InnerWorkings Inc.": "INWK",
  "Ionis Pharmaceuticals Inc.": "IONS",
  "Innospec Inc.": "IOSP",
  "Adesto Technologies Corporation": "IOTS",
  "Iovance Biotherapeutics Inc.": "IOVA",
  "Inter Parfums Inc.": "IPAR",
  "iPass Inc.": "IPAS",
  "Infinity Property and Casualty Corporation": "IPCC",
  "Intellipharmaceutics International Inc.": "IPCI",
  "Professional Diversity Network Inc.": "IPDN",
  "IPG Photonics Corporation": "IPGP",
  "Innophos Holdings Inc.": "IPHS",
  "PowerShares International BuyBack Achievers Portfolio": "IPKW",
  "Ideal Power Inc.": "IPWR",
  "Impax Laboratories Inc.": "IPXL",
  "iRobot Corporation": "IRBT",
  "IRSA Propiedades Comerciales S.A.": "IRCP",
  "Iridium Communications Inc": "IRDM",
  "Iridium Communications Inc 6.75% Series B Cumulative Perpetual Convertible Preferred Stock": "IRDMB",
  "IRIDEX Corporation": "IRIX",
  "iRadimed Corporation": "IRMD",
  "IF Bancorp Inc.": "IROQ",
  "iRhythm Technologies Inc.": "IRTC",
  "Ironwood Pharmaceuticals Inc.": "IRWD",
  "Investors Bancorp Inc.": "ISBC",
  "International Speedway Corporation": "ISCA",
  "iShares 1-3 Year International Treasury Bond ETF": "ISHG",
  "Insignia Systems Inc.": "ISIG",
  "Navient Corporation": "ISM",
  "Image Sensing Systems Inc.": "ISNS",
  "Intuitive Surgical Inc.": "ISRG",
  "Isramco Inc.": "ISRL",
  "Innovative Solutions and Support Inc.": "ISSC",
  "Investar Holding Corporation": "ISTR",
  "Intra-Cellular Therapies Inc.": "ITCI",
  "ETF Managers Trust BlueStar TA-BIGITech Israel Technology": "ITEQ",
  "Investors Title Company": "ITIC",
  "Itron Inc.": "ITRI",
  "Ituran Location and Control Ltd.": "ITRN",
  "ITUS Corporation": "ITUS",
  "Intevac Inc.": "IVAC",
  "Ivy NextShares": "IVFVC",
  "Invuity Inc.": "IVTY",
  "IXYS Corporation": "IXYS",
  "IZEA Inc.": "IZEA",
  "Jack In The Box Inc.": "JACK",
  "Jaguar Health Inc.": "JAGX",
  "JAKKS Pacific Inc.": "JAKK",
  "Jason Industries Inc.": "JASN",
  "Jason Industries Inc. Warrant": "JASNW",
  "JA Solar Holdings Co. Ltd.": "JASO",
  "J. Alexander's Holdings Inc.": "JAX",
  "Jazz Pharmaceuticals plc": "JAZZ",
  "J.B. Hunt Transport Services Inc.": "JBHT",
  "JetBlue Airways Corporation": "JBLU",
  "John B. Sanfilippo & Son Inc.": "JBSS",
  "Nuveen Credit Opportunities 2022 Target Term Fund of Beneficial Interest": "JCO",
  "j2 Global Inc.": "JCOM",
  "Communications Systems Inc.": "JCS",
  "Jewett-Cameron Trading Company": "JCTCF",
  "JD.com Inc.": "JD",
  "J & J Snack Foods Corp.": "JJSF",
  "Jack Henry & Associates Inc.": "JKHY",
  "Jamba Inc.": "JMBA",
  "JMU Limited": "JMU",
  "Jounce Therapeutics Inc.": "JNCE",
  "Juniper Pharmaceuticals Inc.": "JNP",
  "51job Inc.": "JOBS",
  "Johnson Outdoors Inc.": "JOUT",
  "China Finance Online Co. Limited": "JRJC",
  "James River Group Holdings Ltd.": "JRVR",
  "Janus Henderson Small/Mid Cap Growth Alpha ETF": "JSMD",
  "Janus Henderson Small Cap Growth Alpha ETF": "JSML",
  "Jensyn Acquistion Corp.": "JSYN",
  "Jensyn Acquistion Corp. Rights": "JSYNR",
  "Jensyn Acquistion Corp. Unit": "JSYNU",
  "Jensyn Acquistion Corp. Warrants": "JSYNW",
  "JetPay Corporation": "JTPY",
  "Juno Therapeutics Inc.": "JUNO",
  "Coffee Holding Co. Inc.": "JVA",
  "Jacksonville Bancorp Inc.": "JXSB",
  "The Joint Corp.": "JYNT",
  "Kayne Anderson Acquisition Corp.": "KAAC",
  "Kayne Anderson Acquisition Corp. Unit": "KAACU",
  "Kayne Anderson Acquisition Corp. Warrant": "KAACW",
  "Kala Pharmaceuticals Inc.": "KALA",
  "Kaiser Aluminum Corporation": "KALU",
  "KalVista Pharmaceuticals Inc.": "KALV",
  "iKang Healthcare Group Inc.": "KANG",
  "Kimball International Inc.": "KBAL",
  "KBL Merger Corp. IV": "KBLM",
  "KBL Merger Corp. IV Right": "KBLMR",
  "KBL Merger Corp. IV Unit": "KBLMU",
  "KBL Merger Corp. IV Warrant": "KBLMW",
  "KBS Fashion Group Limited": "KBSF",
  "KCAP Financial Inc.": "KCAP",
  "KCAP Financial Inc. 6.125% Notes due 2022": "KCAPL",
  "Kimball Electronics Inc.": "KE",
  "Kelly Services Inc. Class A Common Stock": "KELYA",
  "Kelly Services Inc. Class B Common Stock": "KELYB",
  "Kewaunee Scientific Corporation": "KEQU",
  "Keryx Biopharmaceuticals Inc.": "KERX",
  "The KEYW Holding Corporation": "KEYW",
  "Kentucky First Federal Bancorp": "KFFB",
  "Kforce Inc.": "KFRC",
  "Kingold Jewelry Inc.": "KGJI",
  "The Kraft Heinz Company": "KHC",
  "OrthoPediatrics Corp.": "KIDS",
  "Kindred Biosciences Inc.": "KIN",
  "Kingstone Companies Inc": "KINS",
  "Kirkland's Inc.": "KIRK",
  "KLA-Tencor Corporation": "KLAC",
  "Kulicke and Soffa Industries Inc.": "KLIC",
  "KLX Inc.": "KLXI",
  "Kamada Ltd.": "KMDA",
  "KemPharm Inc.": "KMPH",
  "Kandi Technologies Group Inc.": "KNDI",
  "Kinsale Capital Group Inc.": "KNSL",
  "Kona Grill Inc.": "KONA",
  "Kingtone Wirelessinfo Solution Holding Ltd": "KONE",
  "Cesca Therapeutics Inc.": "KOOL",
  "Kopin Corporation": "KOPN",
  "Koss Corporation": "KOSS",
  "Karyopharm Therapeutics Inc.": "KPTI",
  "Global X Conscious Companies ETF": "KRMA",
  "Kornit Digital Ltd.": "KRNT",
  "Kearny Financial": "KRNY",
  "Krystal Biotech Inc.": "KRYS",
  "Key Tronic Corporation": "KTCC",
  "Key Technology Inc.": "KTEC",
  "Kratos Defense & Security Solutions Inc.": "KTOS",
  "Kitov Pharmaceuticals Holdings Ltd.": "KTOV",
  "Kitov Pharmaceuticals Holdings Ltd. Warrants": "KTOVW",
  "K2M Group Holdings Inc.": "KTWO",
  "Kura Oncology Inc.": "KURA",
  "KVH Industries Inc.": "KVHI",
  "KraneShares Trust CSI China Internet": "KWEB",
  "Kazia Therapeutics Limited": "KZIA",
  "Multi-Color Corporation": "LABL",
  "Leisure Acquisition Corp. Unit": "LACQU",
  "Leisure Acquisition Corp. Warrant": "LACQW",
  "Lakeland Industries Inc.": "LAKE",
  "PowerShares Multi-Strategy Alternative Portfolio": "LALT",
  "Lamar Advertising Company": "LAMR",
  "Lancaster Colony Corporation": "LANC",
  "Gladstone Land Corporation": "LAND",
  "Gladstone Land Corporation 6.375% Series A Cumulative Term Preferred Stock": "LANDP",
  "Landmark Bancorp Inc.": "LARK",
  "Laureate Education Inc.": "LAUR",
  "Lawson Products Inc.": "LAWS",
  "Layne Christensen Company": "LAYN",
  "Lakeland Bancorp Inc.": "LBAI",
  "Luther Burbank Corporation": "LBC",
  "Long Blockchain Corp.": "LBCC",
  "Leading Brands Inc": "LBIX",
  "Liberty Broadband Corporation Class A Common Stock": "LBRDA",
  "Liberty Broadband Corporation": "LBRDK",
  "Liberty Global plc Class A Ordinary Shares": "LBTYA",
  "Liberty Global plc Class B Ordinary Shares": "LBTYB",
  "Liberty Global plc": "LBTYK",
  "Landcadia Holdings Inc.": "LCA",
  "Landcadia Holdings Inc. Unit": "LCAHU",
  "Landcadia Holdings Inc. Warrant": "LCAHW",
  "LCNB Corporation": "LCNB",
  "Lifetime Brands Inc.": "LCUT",
  "PowerShares LadderRite 0-5 Year Corporate Bond Portfolio": "LDRI",
  "Lands' End Inc.": "LE",
  "Lincoln Electric Holdings Inc.": "LECO",
  "SemiLEDS Corporation": "LEDS",
  "Presbia PLC": "LENS",
  "Liberty Expedia Holdings Inc. Series A Common Stock": "LEXEA",
  "Liberty Expedia Holdings Inc. Series B Common Stock": "LEXEB",
  "LongFin Corp": "LFIN",
  "Littelfuse Inc.": "LFUS",
  "Lifevantage Corporation": "LFVN",
  "Legacy Reserves LP": "LGCY",
  "Legacy Reserves LP 8.00% Series B Fixed-to-Floating Rate Cumulative Redeemable Perpetual Preferred Units": "LGCYO",
  "Legacy Reserves LP 8% Series A Fixed-to-Floating Rate Cumulative Redeemable Perpetual Preferred Units": "LGCYP",
  "LGI Homes Inc.": "LGIH",
  "Ligand Pharmaceuticals Incorporated": "LGND",
  "LHC Group": "LHCG",
  "aTyr Pharma Inc.": "LIFE",
  "Liberty Latin America Ltd.": "LILA",
  "Lincoln Educational Services Corporation": "LINC",
  "Lindblad Expeditions Holdings Inc.": "LIND",
  "Lindblad Expeditions Holdings Inc. Warrant": "LINDW",
  "Interlink Electronics Inc.": "LINK",
  "LiNiu Technology Group": "LINU",
  "Fidelity Southern Corporation": "LION",
  "Lumentum Holdings Inc.": "LITE",
  "Live Ventures Incorporated": "LIVE",
  "LivaNova PLC": "LIVN",
  "La Jolla Pharmaceutical Company": "LJPC",
  "Lakeland Financial Corporation": "LKFN",
  "FlexShares Credit-Scored US Long Corporate Bond Index Fund": "LKOR",
  "LKQ Corporation": "LKQ",
  "Lilis Energy Inc.": "LLEX",
  "Lianluo Smart Limited": "LLIT",
  "Limelight Networks Inc.": "LLNW",
  "LeMaitre Vascular Inc.": "LMAT",
  "Limbach Holdings Inc.": "LMB",
  "First Trust Low Duration Opportunities ETF": "LMBS",
  "LM Funding America Inc.": "LMFA",
  "LM Funding America Inc. Warrant": "LMFAW",
  "Limoneira Co": "LMNR",
  "Luminex Corporation": "LMNX",
  "Landmark Infrastructure Partners LP": "LMRK",
  "Landmark Infrastructure Partners LP Preferred Units": "LMRKO",
  "Landmark Infrastructure Partners LP 8.00% Series A Cumulative Redeemable Perpetual Preferred Units": "LMRKP",
  "Snyder's-Lance Inc.": "LNCE",
  "Landec Corporation": "LNDC",
  "Global X Longevity Thematic ETF": "LNGR",
  "Lantheus Holdings Inc.": "LNTH",
  "Manhattan Bridge Capital Inc": "LOAN",
  "Live Oak Bancshares Inc.": "LOB",
  "El Pollo Loco Holdings Inc.": "LOCO",
  "Logitech International S.A.": "LOGI",
  "LogMeIn Inc.": "LOGM",
  "Lonestar Resources US Inc.": "LONE",
  "Loop Industries Inc.": "LOOP",
  "Grand Canyon Education Inc.": "LOPE",
  "Loral Space and Communications Inc.": "LORL",
  "Loxo Oncology Inc.": "LOXO",
  "Lipocine Inc.": "LPCN",
  "LPL Financial Holdings Inc.": "LPLA",
  "LifePoint Health Inc.": "LPNT",
  "LivePerson Inc.": "LPSN",
  "LightPath Technologies Inc.": "LPTH",
  "Leap Therapeutics Inc.": "LPTX",
  "Liquidity Services Inc.": "LQDT",
  "LRAD Corporation": "LRAD",
  "Lam Research Corporation": "LRCX",
  "ClearBridge Large Cap Growth ESG ETF": "LRGE",
  "Lake Shore Bancorp Inc.": "LSBK",
  "Lattice Semiconductor Corporation": "LSCC",
  "Landstar System Inc.": "LSTR",
  "Liberty Media Corporation Series A Liberty SiriusXM Common Stock": "LSXMA",
  "Liberty Media Corporation Series B Liberty SiriusXM Common Stock": "LSXMB",
  "Liberty Media Corporation Series C Liberty SiriusXM Common Stock": "LSXMK",
  "Lightbridge Corporation": "LTBR",
  "Liberty TripAdvisor Holdings Inc. Series A Common Stock": "LTRPA",
  "Liberty TripAdvisor Holdings Inc. Series B Common Stock": "LTRPB",
  "Lantronix Inc.": "LTRX",
  "LegacyTexas Financial Group Inc.": "LTXB",
  "lululemon athletica inc.": "LULU",
  "Luna Innovations Incorporated": "LUNA",
  "Legg Mason Low Volatility High Dividend ETF": "LVHD",
  "Liberty Interactive Corporation Series A Liberty Ventures Common Stock": "LVNTA",
  "Liberty Interactive Corporation Series B Liberty Ventures Common Stock": "LVNTB",
  "Lifeway Foods Inc.": "LWAY",
  "LexinFintech Holdings Ltd.": "LX",
  "Lexicon Pharmaceuticals Inc.": "LXRX",
  "Dragon Victory International Limited": "LYL",
  "LSI Industries Inc.": "LYTS",
  "Merrimack Pharmaceuticals Inc.": "MACK",
  "M I Acquisitions Inc.": "MACQ",
  "M I Acquisitions Inc. Unit": "MACQU",
  "M I Acquisitions Inc. Warrant": "MACQW",
  "Magal Security Systems Ltd.": "MAGS",
  "MAM Software Group Inc.": "MAMS",
  "Manhattan Associates Inc.": "MANH",
  "ManTech International Corporation": "MANT",
  "Marriott International": "MAR",
  "Marathon Patent Group Inc.": "MARA",
  "Remark Holdings Inc.": "MARK",
  "Marine Petroleum Trust Units of Beneficial Interest": "MARPS",
  "Masimo Corporation": "MASI",
  "Mattel Inc.": "MAT",
  "Mattersight Corporation": "MATR",
  "Matthews International Corporation": "MATW",
  "J. W. Mays Inc.": "MAYS",
  "MINDBODY Inc.": "MB",
  "Middlefield Banc Corp.": "MBCN",
  "MB Financial Inc.": "MBFI",
  "MB Financial Inc. Depositary Shares Each Representing a 1/40th Interest in a Share of 6.000% Noncumulative Perpetual Preferred Stock Series C":
    "MBFIO",
  "MB Financial Inc. Perpetual Non-Cumulative Preferred Stock Series A": "MBFIP",
  "Marrone Bio Innovations Inc.": "MBII",
  "Merchants Bancorp": "MBIN",
  "Mustang Bio Inc.": "MBIO",
  "Microbot Medical Inc.": "MBOT",
  "Moleculin Biotech Inc.": "MBRX",
  "FlexShares Disciplined Duration MBS Index Fund": "MBSD",
  "M B T Financial Corp": "MBTF",
  "Malibu Boats Inc.": "MBUU",
  "MabVax Therapeutics Holdings Inc.": "MBVX",
  "Mercantile Bank Corporation": "MBWM",
  "Macatawa Bank Corporation": "MCBC",
  "First Trust Municipal CEF Income Opportunity ETF": "MCEF",
  "Mid-Con Energy Partners LP": "MCEP",
  "MCBC Holdings Inc.": "MCFT",
  "Microchip Technology Incorporated": "MCHP",
  "Marchex Inc.": "MCHX",
  "Seres Therapeutics Inc.": "MCRB",
  "Monarch Casino & Resort Inc.": "MCRI",
  "MongoDB Inc.": "MDB",
  "MDC Partners Inc.": "MDCA",
  "The Medicines Company": "MDCO",
  "Madrigal Pharmaceuticals Inc.": "MDGL",
  "Medigus Ltd.": "MDGS",
  "First Trust Multi-Asset Diversified Income Index Fund": "MDIV",
  "Mondelez International Inc.": "MDLZ",
  "Allscripts Healthcare Solutions Inc.": "MDRX",
  "Medidata Solutions Inc.": "MDSO",
  "MediWound Ltd.": "MDWD",
  "MiMedx Group Inc": "MDXG",
  "Medpace Holdings Inc.": "MEDP",
  "The Meet Group Inc.": "MEET",
  "MEI Pharma Inc.": "MEIP",
  "MercadoLibre Inc.": "MELI",
  "Melrose Bancorp Inc.": "MELR",
  "Methanex Corporation": "MEOH",
  "Mercer International Inc.": "MERC",
  "Mesoblast Limited": "MESO",
  "Ramaco Resources Inc.": "METC",
  "Medallion Financial Corp.": "MFIN",
  "Medallion Financial Corp. 9.000% Notes due 2021": "MFINL",
  "Mackinac Financial Corporation": "MFNC",
  "MutualFirst Financial Inc.": "MFSF",
  "MGC Diagnostics Corporation": "MGCD",
  "MGE Energy Inc.": "MGEE",
  "Miragen Therapeutics Inc.": "MGEN",
  "Moneygram International Inc.": "MGI",
  "Magic Software Enterprises Ltd.": "MGIC",
  "Magellan Health Inc.": "MGLN",
  "MacroGenics Inc.": "MGNX",
  "MGP Ingredients Inc.": "MGPI",
  "McGrath RentCorp": "MGRC",
  "Magyar Bancorp Inc.": "MGYR",
  "Maiden Holdings Ltd.": "MHLD",
  "Micronet Enertec Technologies Inc.": "MICT",
  "Micronet Enertec Technologies Inc. Warrant": "MICTW",
  "The Middleby Corporation": "MIDD",
  "M III Acquisition Corp.": "MIII",
  "M III Acquisition Corp. Unit": "MIIIU",
  "M III Acquisition Corp. Warrant": "MIIIW",
  "The Michaels Companies Inc.": "MIK",
  "Global X Millennials Thematic ETF": "MILN",
  "Mimecast Limited": "MIME",
  "Mitcham Industries Inc.": "MIND",
  "Mitcham Industries Inc. Series A 9.00% Series A Cumulative Preferred Stock": "MINDP",
  "Mobile Mini Inc.": "MINI",
  "Mitek Systems Inc.": "MITK",
  "Mitel Networks Corporation": "MITL",
  "MKS Instruments Inc.": "MKSI",
  "MarketAxess Holdings Inc.": "MKTX",
  "Mesa Laboratories Inc.": "MLAB",
  "Melco Resorts & Entertainment Limited": "MLCO",
  "Herman Miller Inc.": "MLHR",
  "ModusLink Global Solutions Inc": "MLNK",
  "Melinta Therapeutics Inc.": "MLNT",
  "Mellanox Technologies Ltd.": "MLNX",
  "Malvern Bancorp Inc.": "MLVF",
  "MMA Capital Management LLC": "MMAC",
  "Modern Media Acquisition Corp.": "MMDM",
  "Modern Media Acquisition Corp. Right": "MMDMR",
  "Modern Media Acquisition Corp. Unit": "MMDMU",
  "Modern Media Acquisition Corp. Warrant": "MMDMW",
  "Martin Midstream Partners L.P.": "MMLP",
  "Merit Medical Systems Inc.": "MMSI",
  "MakeMyTrip Limited": "MMYT",
  "MIND C.T.I. Ltd.": "MNDO",
  "MagneGas Corporation": "MNGA",
  "MannKind Corporation": "MNKD",
  "MediciNova Inc.": "MNOV",
  "Monro Inc.": "MNRO",
  "Monster Beverage Corporation": "MNST",
  "Momenta Pharmaceuticals Inc.": "MNTA",
  "Manitex International Inc.": "MNTX",
  "MobileIron Inc.": "MOBL",
  "MidWestOne Financial Group Inc.": "MOFG",
  "Gabelli Media Mogul NextShares": "MOGLCNX",
  "Momo Inc.": "MOMO",
  "Morningstar Inc.": "MORN",
  "MoSys Inc.": "MOSY",
  "Moxian Inc.": "MOXC",
  "Motorcar Parts of America Inc.": "MPAA",
  "Matlin & Partners Acquisition Corporation": "MPAC",
  "Matlin & Partners Acquisition Corporation Unit": "MPACU",
  "Matlin & Partners Acquisition Corporation Warrants": "MPACW",
  "Mid Penn Bancorp": "MPB",
  "iShares MSCI Global Impact ETF": "MPCT",
  "Monolithic Power Systems Inc.": "MPWR",
  "Everspin Technologies Inc.": "MRAM",
  "Meridian Bank": "MRBK",
  "Monroe Capital Corporation": "MRCC",
  "Mercury Systems Inc": "MRCY",
  "Meridian Waste Solutions Inc": "MRDN",
  "Meridian Waste Solutions Inc Warrants": "MRDNW",
  "Marlin Business Services Corp.": "MRLN",
  "Marinus Pharmaceuticals Inc.": "MRNS",
  "Mersana Therapeutics Inc.": "MRSN",
  "Marten Transport Ltd.": "MRTN",
  "Mirati Therapeutics Inc.": "MRTX",
  "Merus N.V.": "MRUS",
  "Marvell Technology Group Ltd.": "MRVL",
  "MSB Financial Corp.": "MSBF",
  "Midland States Bancorp Inc.": "MSBI",
  "Microsemi Corporation": "MSCC",
  "Monster Digital Inc.": "MSDI",
  "Monster Digital Inc. Warrant": "MSDIW",
  "Middlesex Water Company": "MSEX",
  "MainSource Financial Group Inc.": "MSFG",
  "Microsoft Corporation": "MSFT",
  "The Madison Square Garden Company Class A (New)": "MSG",
  "MISONIX Inc.": "MSON",
  "MicroStrategy Incorporated": "MSTR",
  "Medical Transcription Billing Corp.": "MTBC",
  "Medical Transcription Billing Corp. 11% Series A Cumulative Redeemable Perpetual Preferred Stock": "MTBCP",
  "Match Group Inc.": "MTCH",
  "Molecular Templates Inc.": "MTEM",
  "Mannatech Incorporated": "MTEX",
  "Motif Bio plc": "MTFB",
  "Motif Bio plc Warrant": "MTFBW",
  "MTGE Investment Corp.": "MTGE",
  "MTGE Investment Corp. 8.125% Series A Cumulative Redeemable Preferred Stock": "MTGEP",
  "Materialise NV": "MTLS",
  "Matrix Service Company": "MTRX",
  "MTS Systems Corporation": "MTSC",
  "MACOM Technology Solutions Holdings Inc.": "MTSI",
  "MER Telemanagement Solutions Ltd.": "MTSL",
  "Micron Technology Inc.": "MU",
  "MVB Financial Corp.": "MVBF",
  "Microvision Inc.": "MVIS",
  "Maxim Integrated Products Inc.": "MXIM",
  "Maxwell Technologies Inc.": "MXWL",
  "Myriad Genetics Inc.": "MYGN",
  "Mylan N.V.": "MYL",
  "MYnd Analytics Inc.": "MYND",
  "MYnd Analytics Inc. Warrant": "MYNDW",
  "MyoKardia Inc.": "MYOK",
  "MYOS RENS Technology Inc.": "MYOS",
  "MYR Group Inc.": "MYRG",
  "My Size Inc.": "MYSZ",
  "Mazor Robotics Ltd.": "MZOR",
  "Natural Alternatives International Inc.": "NAII",
  "Naked Brand Group Inc.": "NAKD",
  "Nanometrics Incorporated": "NANO",
  "NanoVibronix Inc.": "NAOV",
  "Nathan's Famous Inc.": "NATH",
  "National Instruments Corporation": "NATI",
  "Nature's Sunshine Products Inc.": "NATR",
  "National American University Holdings Inc.": "NAUH",
  "The Navigators Group Inc.": "NAVG",
  "New Age Beverages Corporation": "NBEV",
  "Neurocrine Biosciences Inc.": "NBIX",
  "Northeast Bancorp": "NBN",
  "Nabriva Therapeutics plc": "NBRV",
  "NBT Bancorp Inc.": "NBTB",
  "Nicolet Bankshares Inc.": "NCBS",
  "Norwegian Cruise Line Holdings Ltd.": "NCLH",
  "National CineMedia Inc.": "NCMI",
  "NuCana plc": "NCNA",
  "National Commerce Corporation": "NCOM",
  "NCS Multistage Holdings Inc.": "NCSM",
  "The9 Limited": "NCTY",
  "Nasdaq Inc.": "NDAQ",
  "Noodles & Company": "NDLS",
  "ENDRA Life Sciences Inc.": "NDRA",
  "ENDRA Life Sciences Inc. Warrants": "NDRAW",
  "Nordson Corporation": "NDSN",
  "Nebula Acquisition Corporation Unit": "NEBUU",
  "NeoGenomics Inc.": "NEO",
  "Neogen Corporation": "NEOG",
  "Neonode Inc.": "NEON",
  "Neos Therapeutics Inc.": "NEOS",
  "Neptune Technologies & Bioresources Inc": "NEPT",
  "Minerva Neurosciences Inc": "NERV",
  "National Energy Services Reunited Corp.": "NESR",
  "National Energy Services Reunited Corp. Warrant": "NESRW",
  "Net Element Inc.": "NETE",
  "Newater Technology Inc.": "NEWA",
  "NewStar Financial Inc.": "NEWS",
  "Newtek Business Services Corp.": "NEWT",
  "Newtek Business Services Corp. 7.00% Notes Due 2021": "NEWTL",
  "Newtek Business Services Corp. Notes due 2022": "NEWTZ",
  "NextDecade Corporation": "NEXT",
  "NextDecade Corporation Warrant": "NEXTW",
  "Northfield Bancorp Inc.": "NFBK",
  "NF Energy Saving Corporation": "NFEC",
  "Netflix Inc.": "NFLX",
  "National General Holdings Corp": "NGHC",
  "National General Holdings Corp Depositary Shares each representing 1/40th of a share of 7.50% Non-Cumulative Preferred Stock Series C":
    "NGHCN",
  "National General Holdings Corp Depositary Shares": "NGHCO",
  "National General Holdings Corp 7.50% Non-Cumulative Preferred Stock Series A": "NGHCP",
  "National General Holdings Corp 7.625% Subordinated Notes due 2055": "NGHCZ",
  "NantHealth Inc.": "NH",
  "National Holdings Corporation": "NHLD",
  "National Holdings Corporation Warrants": "NHLDW",
  "Natural Health Trends Corp.": "NHTC",
  "NICE Ltd": "NICE",
  "Nicholas Financial Inc.": "NICK",
  "NII Holdings Inc.": "NIHD",
  "Nine Energy Service Inc - Ordinary Shares": "NINE",
  "Nightstar Therapeutics plc": "NITE",
  "NantKwest Inc.": "NK",
  "National Bankshares Inc.": "NKSH",
  "Nektar Therapeutics": "NKTR",
  "NewLink Genetics Corporation": "NLNK",
  "Netlist Inc.": "NLST",
  "NMI Holdings Inc": "NMIH",
  "Newmark Group Inc.": "NMRK",
  "Numerex Corp.": "NMRX",
  "NN Inc.": "NNBR",
  "Nano Dimension Ltd.": "NNDM",
  "NI Holdings Inc.": "NODK",
  "Novan Inc.": "NOVN",
  "Novanta Inc.": "NOVT",
  "National Research Corporation Class A Common Stock": "NRCIA",
  "National Research Corporation Common Stock": "NRCIB",
  "Northrim BanCorp Inc": "NRIM",
  "National Security Group Inc.": "NSEC",
  "Insight Enterprises Inc.": "NSIT",
  "NAPCO Security Technologies Inc.": "NSSC",
  "NanoString Technologies Inc.": "NSTG",
  "Nortech Systems Incorporated": "NSYS",
  "NetApp Inc.": "NTAP",
  "NetScout Systems Inc.": "NTCT",
  "Intec Pharma Ltd.": "NTEC",
  "NetEase Inc.": "NTES",
  "NETGEAR Inc.": "NTGR",
  "Northern Technologies International Corporation": "NTIC",
  "Intellia Therapeutics Inc.": "NTLA",
  "Nutanix Inc.": "NTNX",
  "Natera Inc.": "NTRA",
  "NutriSystem Inc": "NTRI",
  "Neurotrope Inc.": "NTRP",
  "Northern Trust Corporation": "NTRS",
  "Northern Trust Corporation Depository Shares": "NTRSP",
  "NETSOL Technologies Inc.": "NTWK",
  "Nuance Communications Inc.": "NUAN",
  "NeuroMetrix Inc.": "NURO",
  "NeuroMetrix Inc. Warrants": "NUROW",
  "NuVasive Inc.": "NUVA",
  "Novavax Inc.": "NVAX",
  "Neovasc Inc.": "NVCN",
  "NovoCure Limited": "NVCR",
  "NVIDIA Corporation": "NVDA",
  "NVE Corporation": "NVEC",
  "NV5 Global Inc.": "NVEE",
  "Nova Lifestyle Inc": "NVFY",
  "InVivo Therapeutics Holdings Corp.": "NVIV",
  "Novelion Therapeutics Inc.": "NVLN",
  "Nova Measuring Instruments Ltd.": "NVMI",
  "Novume Solutions Inc.": "NVMM",
  "Nuvectra Corporation": "NVTR",
  "Novus Therapeutics Inc.": "NVUS",
  "Northwest Bancshares Inc.": "NWBI",
  "Norwood Financial Corp.": "NWFL",
  "National Western Life Group Inc.": "NWLI",
  "Northwest Pipe Company": "NWPX",
  "News Corporation": "NWS",
  "Nexeo Solutions Inc.": "NXEO",
  "Nexeo Solutions Inc. Unit": "NXEOU",
  "Nexeo Solutions Inc. Warrant": "NXEOW",
  "NXP Semiconductors N.V.": "NXPI",
  "Nexstar Media Group Inc.": "NXST",
  "NXT-ID Inc.": "NXTD",
  "NXT-ID Inc. Warrant": "NXTDW",
  "NxStage Medical Inc.": "NXTM",
  "New York Mortgage Trust Inc.": "NYMT",
  "New York Mortgage Trust Inc. 8.00% Series D Fixed-to-Floating Rate Cumulative Redeemable Preferred Stock": "NYMTN",
  "New York Mortgage Trust Inc. 7.875% Series C Cumulative Redeemable Preferred Stock": "NYMTO",
  "New York Mortgage Trust Inc. 7.75% Series B Cumulative Redeemable Preferred Stock": "NYMTP",
  "Nymox Pharmaceutical Corporation": "NYMX",
  "Empire Resorts Inc.": "NYNY",
  "Origo Acquisition Corporation": "OACQ",
  "Origo Acquisition Corporation Right Expiring 3/12/2018": "OACQR",
  "Origo Acquisition Corporation Unit": "OACQU",
  "Origo Acquisition Corporation Warrants": "OACQW",
  "Oasmia Pharmaceutical AB": "OASM",
  "Optibase Ltd.": "OBAS",
  "Ocean Bio-Chem Inc.": "OBCI",
  "Obalon Therapeutics Inc.": "OBLN",
  "ObsEva SA": "OBSV",
  "Optical Cable Corporation": "OCC",
  "OceanFirst Financial Corp.": "OCFC",
  "Oclaro Inc.": "OCLR",
  "Ocera Therapeutics Inc.": "OCRX",
  "Oaktree Strategic Income Corporation": "OCSI",
  "Oaktree Specialty Lending Corporation": "OCSL",
  "Oaktree Specialty Lending Corporation 6.125% senior notes due 2028": "OCSLL",
  "Ocular Therapeutix Inc.": "OCUL",
  "Old Dominion Freight Line Inc.": "ODFL",
  "Office Depot Inc.": "ODP",
  "Odonate Therapeutics Inc.": "ODT",
  "Oconee Federal Financial Corp.": "OFED",
  "Orthofix International N.V.": "OFIX",
  "Omega Flex Inc.": "OFLX",
  "OFS Capital Corporation": "OFS",
  "OHA Investment Corporation": "OHAI",
  "One Horizon Group Inc.": "OHGI",
  "Ohr Pharmaceuticals Inc.": "OHRP",
  "O2Micro International Limited": "OIIM",
  "ProShares UltraPro 3x Short Crude Oil": "OILD",
  "ProShares UltraPro 3x Crude Oil": "OILU",
  "Eaton Vance Oaktree Diversified Credit NextShares": "OKDCCNX",
  "Okta Inc.": "OKTA",
  "Old Line Bancshares Inc.": "OLBK",
  "The Long-Term Care ETF": "OLD",
  "Universal Display Corporation": "OLED",
  "Ollie's Bargain Outlet Holdings Inc.": "OLLI",
  "Grupo Aeroportuario del Centro Norte S.A.B. de C.V.": "OMAB",
  "Omnicell Inc.": "OMCL",
  "OncoMed Pharmaceuticals Inc.": "OMED",
  "Omeros Corporation": "OMER",
  "Odyssey Marine Exploration Inc.": "OMEX",
  "Ominto Inc.": "OMNT",
  "ON Semiconductor Corporation": "ON",
  "Old National Bancorp": "ONB",
  "Spark Therapeutics Inc.": "ONCE",
  "OncoSec Medical Incorporated": "ONCS",
  "Fidelity Nasdaq Composite Index Tracking Stock": "ONEQ",
  "Oncobiologics Inc.": "ONS",
  "Oncobiologics Inc. Series A Warrant": "ONSIW",
  "Oncobiologics Inc. Series B Warrant": "ONSIZ",
  "Onconova Therapeutics Inc.": "ONTX",
  "Onconova Therapeutics Inc. Warrants": "ONTXW",
  "Opus Bank": "OPB",
  "OpGen Inc.": "OPGN",
  "OpGen Inc. Warrant": "OPGNW",
  "OptimumBank Holdings Inc.": "OPHC",
  "Ophthotech Corporation": "OPHT",
  "Opiant Pharmaceuticals Inc.": "OPNT",
  "Old Point Financial Corporation": "OPOF",
  "OptiNose Inc.": "OPTN",
  "Ocean Power Technologies Inc.": "OPTT",
  "ORBCOMM Inc.": "ORBC",
  "Orbotech Ltd.": "ORBK",
  "Orexigen Therapeutics Inc.": "OREX",
  "The Organics ETF": "ORG",
  "Ocean Rig UDW Inc.": "ORIG",
  "Oritani Financial Corp.": "ORIT",
  "O'Reilly Automotive Inc.": "ORLY",
  "Oramed Pharmaceuticals Inc.": "ORMP",
  "Bioblast Pharma Ltd.": "ORPN",
  "Orrstown Financial Services Inc": "ORRF",
  "Old Second Bancorp Inc.": "OSBC",
  "Old Second Bancorp Inc. 7.80% Cumulative Trust Preferred Securities": "OSBCP",
  "OSI Systems Inc.": "OSIS",
  "Ossen Innovation Co. Ltd.": "OSN",
  "Osprey Energy Acquisition Corp.": "OSPR",
  "Osprey Energy Acquisition Corp. Unit": "OSPRU",
  "Osprey Energy Acquisition Corp. Warrant": "OSPRW",
  "Overstock.com Inc.": "OSTK",
  "OraSure Technologies Inc.": "OSUR",
  "Otelco Inc.": "OTEL",
  "Open Text Corporation": "OTEX",
  "Otonomy Inc.": "OTIC",
  "On Track Innovations Ltd": "OTIV",
  "Otter Tail Corporation": "OTTR",
  "Ottawa Bancorp Inc.": "OTTW",
  "OvaScience Inc.": "OVAS",
  "Ohio Valley Banc Corp.": "OVBC",
  "Ovid Therapeutics Inc.": "OVID",
  "Oak Valley Bancorp (CA)": "OVLY",
  "Oxbridge Re Holdings Limited": "OXBR",
  "Oxbridge Re Holdings Limited Warrant": "OXBRW",
  "Oxford Immunotec Global PLC": "OXFD",
  "Oxford Lane Capital Corp.": "OXLC",
  "Oxford Lane Capital Corp. 6.75% Series 2024 Term Preferred Stock": "OXLCM",
  "Oxford Lane Capital Corp. Term Preferred Shares 7.50% Series 2023": "OXLCO",
  "Bank of the Ozarks": "OZRK",
  "Pan American Silver Corp.": "PAAS",
  "Pacific Biosciences of California Inc.": "PACB",
  "PacWest Bancorp": "PACW",
  "PowerShares Global Agriculture Portfolio": "PAGG",
  "Phibro Animal Health Corporation": "PAHC",
  "Pangaea Logistics Solutions Ltd.": "PANL",
  "Patriot Transportation Holding Inc.": "PATI",
  "Patrick Industries Inc.": "PATK",
  "PAVmed Inc.": "PAVM",
  "PAVmed Inc. Warrant": "PAVMW",
  "Paychex Inc.": "PAYX",
  "PB Bancorp Inc.": "PBBI",
  "People's United Financial Inc.": "PBCT",
  "People's United Financial Inc. Perpetual Preferred Series A Fixed-to-floating Rate": "PBCTP",
  "Pathfinder Bancorp Inc.": "PBHC",
  "Porter Bancorp Inc.": "PBIB",
  "Prudential Bancorp Inc.": "PBIP",
  "Paragon Commercial Corporation": "PBNC",
  "Potbelly Corporation": "PBPB",
  "Poage Bankshares Inc.": "PBSK",
  "PACCAR Inc.": "PCAR",
  "Potlatch Corporation": "PCH",
  "The Priceline Group Inc.": "PCLN",
  "PCM Inc.": "PCMI",
  "Pendrell Corporation": "PCO",
  "Points International Ltd.": "PCOM",
  "Pacira Pharmaceuticals Inc.": "PCRX",
  "PCSB Financial Corporation": "PCSB",
  "PC-Tel Inc.": "PCTI",
  "Paylocity Holding Corporation": "PCTY",
  "Park City Group Inc.": "PCYG",
  "Pure Cycle Corporation": "PCYO",
  "PowerShares Optimum Yield Diversified Commodity Strategy No K-1 Portfolio": "PDBC",
  "PDC Energy Inc.": "PDCE",
  "Patterson Companies Inc.": "PDCO",
  "Pro-Dex Inc.": "PDEX",
  "PDF Solutions Inc.": "PDFS",
  "PDL Community Bancorp": "PDLB",
  "PDL BioPharma Inc.": "PDLI",
  "pdvWireless Inc.": "PDVW",
  "Peoples Bancorp of North Carolina Inc.": "PEBK",
  "Peoples Bancorp Inc.": "PEBO",
  "Pegasystems Inc.": "PEGA",
  "Pattern Energy Group Inc.": "PEGI",
  "Pacific Ethanol Inc.": "PEIX",
  "Penn National Gaming Inc.": "PENN",
  "Perion Network Ltd": "PERI",
  "Perry Ellis International Inc.": "PERY",
  "Perma-Fix Environmental Services Inc.": "PESI",
  "PetIQ Inc.": "PETQ",
  "PetMed Express Inc.": "PETS",
  "Aratana Therapeutics Inc.": "PETX",
  "TDH Holdings Inc.": "PETZ",
  "Preferred Bank": "PFBC",
  "Premier Financial Bancorp Inc.": "PFBI",
  "Peoples Financial Corporation": "PFBX",
  "Profire Energy Inc.": "PFIE",
  "P & F Industries Inc.": "PFIN",
  "Peoples Financial Services Corp.": "PFIS",
  "PennantPark Floating Rate Capital Ltd.": "PFLT",
  "Performant Financial Corporation": "PFMT",
  "Proofpoint Inc.": "PFPT",
  "PFSweb Inc.": "PFSW",
  "Peapack-Gladstone Financial Corporation": "PGC",
  "Pershing Gold Corporation": "PGLC",
  "Progenics Pharmaceuticals Inc.": "PGNX",
  "PGT Innovations Inc.": "PGTI",
  "PHI Inc.": "PHII",
  "Impinj Inc.": "PI",
  "PICO Holdings Inc.": "PICO",
  "1347 Property Insurance Holdings Inc.": "PIH",
  "Premier Inc.": "PINC",
  "Pieris Pharmaceuticals Inc.": "PIRS",
  "ShiftPixy Inc.": "PIXY",
  "Parke Bancorp Inc.": "PKBK",
  "Park-Ohio Holdings Corp.": "PKOH",
  "Photronics Inc.": "PLAB",
  "Dave & Buster's Entertainment Inc.": "PLAY",
  "Plumas Bancorp": "PLBC",
  "Children's Place Inc. (The)": "PLCE",
  "Preformed Line Products Company": "PLPC",
  "Planet Payment Inc.": "PLPM",
  "Pulse Biosciences Inc": "PLSE",
  "Plug Power Inc.": "PLUG",
  "ePlus inc.": "PLUS",
  "PLx Pharma Inc.": "PLXP",
  "Plexus Corp.": "PLXS",
  "Playa Hotels & Resorts N.V.": "PLYA",
  "Pacific Mercantile Bancorp": "PMBC",
  "Psychemedics Corporation": "PMD",
  "Pingtan Marine Enterprise Ltd.": "PME",
  "Principal Sustainable Momentum Index ETF": "PMOM",
  "iSectors Post-MPT Growth ETF": "PMPT",
  "CPI Card Group Inc.": "PMTS",
  "Patriot National Bancorp Inc.": "PNBK",
  "Pinnacle Financial Partners Inc.": "PNFP",
  "PennantPark Investment Corporation": "PNNT",
  "PowerShares Nasdaq Internet Portfolio": "PNQI",
  "PrimeEnergy Corporation": "PNRG",
  "Pointer Telocation Ltd.": "PNTR",
  "Insulet Corporation": "PODD",
  "Polar Power Inc.": "POLA",
  "Pool Corporation": "POOL",
  "Pope Resources": "POPE",
  "Power Integrations Inc.": "POWI",
  "Powell Industries Inc.": "POWL",
  "Pacific Premier Bancorp Inc": "PPBI",
  "Pilgrim's Pride Corporation": "PPC",
  "Peregrine Pharmaceuticals Inc.": "PPHM",
  "Peregrine Pharmaceuticals Inc. 10.50% Series E Convertible Preferred Stock": "PPHMP",
  "Perma-Pipe International Holdings Inc.": "PPIH",
  "Pioneer Power Solutions Inc.": "PPSI",
  "PRA Group Inc.": "PRAA",
  "PRA Health Sciences Inc.": "PRAH",
  "Prana Biotechnology Ltd": "PRAN",
  "Perceptron Inc.": "PRCP",
  "Perficient Inc.": "PRFT",
  "PowerShares FTSE RAFI US 1500 Small-Mid Portfolio": "PRFZ",
  "Progress Software Corporation": "PRGS",
  "PRGX Global Inc.": "PRGX",
  "Primoris Services Corporation": "PRIM",
  "ParkerVision Inc.": "PRKR",
  "Primo Water Corporation": "PRMW",
  "Provident Financial Holdings Inc.": "PROV",
  "ProPhase Labs Inc.": "PRPH",
  "Precipio Inc.": "PRPO",
  "ProQR Therapeutics N.V.": "PRQR",
  "The Providence Service Corporation": "PRSC",
  "CafePress Inc.": "PRSS",
  "Prothena Corporation plc": "PRTA",
  "Paratek Pharmaceuticals Inc.": "PRTK",
  "Proteon Therapeutics Inc.": "PRTO",
  "U.S. Auto Parts Network Inc.": "PRTS",
  "PowerShares Global Gold & Precious Metals Portfolio": "PSAU",
  "Principal U.S. Small-Cap Multi-Factor Index ETF": "PSC",
  "PowerShares S&P SmallCap Consumer Staples Portfolio": "PSCC",
  "PowerShares S&P SmallCap Consumer Discretionary Portfolio": "PSCD",
  "PowerShares S&P SmallCap Energy Portfolio": "PSCE",
  "PowerShares S&P SmallCap Financials Portfolio": "PSCF",
  "PowerShares S&P SmallCap Health Care Portfolio": "PSCH",
  "PowerShares S&P SmallCap Industrials Portfolio": "PSCI",
  "PowerShares S&P SmallCap Materials Portfolio": "PSCM",
  "PowerShares S&P SmallCap Information Technology Portfolio": "PSCT",
  "PowerShares S&P SmallCap Utilities Portfolio": "PSCU",
  "Presidio Inc.": "PSDO",
  "pSivida Corp.": "PSDV",
  "Prospect Capital Corporation": "PSEC",
  "Principal Price Setters Index ETF": "PSET",
  "PriceSmart Inc.": "PSMT",
  "Park Sterling Corporation": "PSTB",
  "Pluristem Therapeutics Inc.": "PSTI",
  "PTC Inc.": "PTC",
  "PTC Therapeutics Inc.": "PTCT",
  "Patterson-UTI Energy Inc.": "PTEN",
  "Protagonist Therapeutics Inc.": "PTGX",
  "Proteostasis Therapeutics Inc.": "PTI",
  "Pain Therapeutics Inc.": "PTIE",
  "Portola Pharmaceuticals Inc.": "PTLA",
  "Partner Communications Company Ltd.": "PTNR",
  "P.A.M. Transportation Services Inc.": "PTSI",
  "Pernix Therapeutics Holdings Inc.": "PTX",
  "People's Utah Bancorp": "PUB",
  "Pulmatrix Inc.": "PULM",
  "Penn Virginia Corporation": "PVAC",
  "Principal Contrarian Value Index ETF": "PVAL",
  "Provident Bancorp Inc.": "PVBC",
  "Penns Woods Bancorp Inc.": "PWOD",
  "Pixelworks Inc.": "PXLW",
  "Pyxis Tankers Inc.": "PXS",
  "Principal International Multi-Factor Index ETF": "PXUS",
  "Principal Shareholder Yield Index ETF": "PY",
  "Payment Data Systems Inc.": "PYDS",
  "PayPal Holdings Inc.": "PYPL",
  "PhaseRx Inc.": "PZRX",
  "Papa John's International Inc.": "PZZA",
  "First Trust NASDAQ ABA Community Bank Index Fund": "QABA",
  "QAD Inc.": "QADA",
  "iShares MSCI Qatar ETF": "QAT",
  "Qualstar Corporation": "QBAK",
  "First Trust NASDAQ Clean Edge Green Energy Index Fund": "QCLN",
  "QUALCOMM Incorporated": "QCOM",
  "QCR Holdings Inc.": "QCRH",
  "Quidel Corporation": "QDEL",
  "Qiagen N.V.": "QGEN",
  "First Trust RBA Quality Income ETF": "QINC",
  "QIWI plc": "QIWI",
  "FlexShares US Quality Large Cap Index Fund": "QLC",
  "Qualys Inc.": "QLYS",
  "QuinStreet Inc.": "QNST",
  "First Trust NASDAQ-100 Equal Weighted Index Fund": "QQEW",
  "PowerShares QQQ Trust Series 1": "QQQ",
  "Global X NASDAQ China Technology ETF": "QQQC",
  "Nuveen NASDAQ 100 Dynamic Overwrite Fund": "QQQX",
  "First Trust NASDAQ-100 Ex-Technology Sector Index Fund": "QQXT",
  "Quest Resource Holding Corporation.": "QRHC",
  "Qorvo Inc.": "QRVO",
  "Quality Systems Inc.": "QSII",
  "First Trust NASDAQ-100- Technology Index Fund": "QTEC",
  "Quantenna Communications Inc.": "QTNA",
  "Quotient Limited": "QTNT",
  "Quarterhill Inc.": "QTRH",
  "Quanterix Corporation": "QTRX",
  "QuickLogic Corporation": "QUIK",
  "Qumu Corporation": "QUMU",
  "uniQure N.V.": "QURE",
  "Liberty Interactive Corporation": "QVCA",
  "Horizons NASDAQ-100 Covered Call ETF": "QYLD",
  "RADA Electronic Industries Ltd.": "RADA",
  "Freightcar America Inc.": "RAIL",
  "Rand Capital Corporation": "RAND",
  "Ultragenyx Pharmaceutical Inc.": "RARE",
  "Ra Pharmaceuticals Inc.": "RARX",
  "Rave Restaurant Group Inc.": "RAVE",
  "Raven Industries Inc.": "RAVN",
  "RBB Bancorp": "RBB",
  "Ribbon Communications Inc.": "RBBN",
  "Republic Bancorp Inc. Class A Common Stock": "RBCAA",
  "Rubicon Technology Inc.": "RBCN",
  "Reliant Bancorp Inc.": "RBNC",
  "Royal Bancshares of Pennsylvania Inc. Class A Common Stock": "RBPAA",
  "Rent-A-Center Inc.": "RCII",
  "Rocket Pharmaceuticals Inc.": "RCKT",
  "Rocky Brands Inc.": "RCKY",
  "R1 RCM Inc.": "RCM",
  "RCM Technologies Inc.": "RCMT",
  "Recon Technology Ltd.": "RCON",
  "Radcom Ltd.": "RDCM",
  "Redfin Corporation": "RDFN",
  "Redhill Biopharma Ltd.": "RDHL",
  "Reading International Inc": "RDI",
  "RadNet Inc.": "RDNT",
  "Radius Health Inc.": "RDUS",
  "First Trust Rising Dividend Achievers ETF": "RDVY",
  "Radware Ltd.": "RDWR",
  "Resources Connection Inc.": "RECN",
  "RISE Education Cayman Ltd": "REDU",
  "Research Frontiers Incorporated": "REFR",
  "Renewable Energy Group Inc.": "REGI",
  "Regeneron Pharmaceuticals Inc.": "REGN",
  "Reis Inc": "REIS",
  "Richardson Electronics Ltd.": "RELL",
  "Reliv' International Inc.": "RELV",
  "Real Industry Inc.": "RELY",
  "Recro Pharma Inc.": "REPH",
  "Resonant Inc.": "RESN",
  "Reata Pharmaceuticals Inc.": "RETA",
  "ReTo Eco-Solutions Inc.": "RETO",
  "Rex Energy Corporation": "REXX",
  "First Trust RiverFront Dynamic Asia Pacific ETF": "RFAP",
  "First Trust RiverFront Dynamic Developed International ETF": "RFDI",
  "First Trust RiverFront Dynamic Emerging Markets ETF": "RFEM",
  "First Trust RiverFront Dynamic Europe ETF": "RFEU",
  "RF Industries Ltd.": "RFIL",
  "RGC Resources Inc.": "RGCO",
  "Repligen Corporation": "RGEN",
  "Royal Gold Inc.": "RGLD",
  "Regulus Therapeutics Inc.": "RGLS",
  "REGENXBIO Inc.": "RGNX",
  "Real Goods Solar Inc.": "RGSE",
  "RiceBran Technologies": "RIBT",
  "RiceBran Technologies Warrant": "RIBTW",
  "RCI Hospitality Holdings Inc.": "RICK",
  "Rigel Pharmaceuticals Inc.": "RIGL",
  "B. Riley Financial Inc.": "RILY",
  "B. Riley Financial Inc. 7.25% Senior Notes due 2027": "RILYG",
  "B. Riley Financial Inc. 7.50% Senior Notes": "RILYL",
  "B. Riley Financial Inc. 7.50% Senior Notes Due 2027": "RILYZ",
  "Riot Blockchain Inc": "RIOT",
  "Arcadia Biosciences Inc.": "RKDA",
  "RLJ Entertainment Inc.": "RLJE",
  "Rand Logistics Inc.": "RLOG",
  "RumbleOn Inc.": "RMBL",
  "Rambus Inc.": "RMBS",
  "Rocky Mountain Chocolate Factory Inc.": "RMCF",
  "RMG Networks Holding Corporation": "RMGN",
  "Rimini Street Inc.": "RMNI",
  "The RMR Group Inc.": "RMR",
  "Rockwell Medical Inc.": "RMTI",
  "Randolph Bancorp Inc.": "RNDB",
  "First Trust Developed International Equity Select ETF": "RNDM",
  "First Trust US Equity Dividend Select ETF": "RNDV",
  "First Trust Emerging Markets Equity Select ETF": "RNEM",
  "RigNet Inc.": "RNET",
  "First Trust Large Cap US Equity Select ETF": "RNLC",
  "First Trust Mid Cap US Equity Select ETF": "RNMC",
  "First Trust Small Cap US Equity Select ETF": "RNSC",
  "Renasant Corporation": "RNST",
  "RealNetworks Inc.": "RNWK",
  "ROBO Global Robotics and Automation Index ETF": "ROBO",
  "Gibraltar Industries Inc.": "ROCK",
  "Retail Opportunity Investments Corp.": "ROIC",
  "Sorrento Tech Inc.": "ROKA",
  "Roku Inc.": "ROKU",
  "RBC Bearings Incorporated": "ROLL",
  "Rosehill Resources Inc.": "ROSE",
  "Rosehill Resources Inc. Unit": "ROSEU",
  "Rosehill Resources Inc. Warrants": "ROSEW",
  "Rosetta Genomics Ltd.": "ROSG",
  "Ross Stores Inc.": "ROST",
  "RealPage Inc.": "RP",
  "Rapid7 Inc.": "RPD",
  "Repros Therapeutics Inc.": "RPRX",
  "RPX Corporation": "RPXC",
  "R.R. Donnelley & Sons Company": "RRD",
  "Red Robin Gourmet Burgers Inc.": "RRGB",
  "ReShape Lifesciences Inc.": "RSLS",
  "RadiSys Corporation": "RSYS",
  "RTI Surgical Inc.": "RTIX",
  "Retrophin Inc.": "RTRX",
  "Ritter Pharmaceuticals Inc.": "RTTR",
  "Sunrun Inc.": "RUN",
  "Rush Enterprises Inc. Class A Common Stock": "RUSHA",
  "Rush Enterprises Inc. Class B Common Stock": "RUSHB",
  "Ruth's Hospitality Group Inc.": "RUTH",
  "Reven Housing REIT Inc.": "RVEN",
  "Revolution Lighting Technologies Inc.": "RVLT",
  "Revance Therapeutics Inc.": "RVNC",
  "Riverview Bancorp Inc": "RVSB",
  "ReWalk Robotics Ltd": "RWLK",
  "Ignyta Inc.": "RXDX",
  "RXi Pharmaceuticals Corporation": "RXII",
  "RXi Pharmaceuticals Corporation Warrants": "RXIIW",
  "Ryanair Holdings plc American Depositary Shares each representing five Ordinary Shares": "RYAAY",
  "Rhythm Pharmaceuticals Inc.": "RYTM",
  "Sabre Corporation": "SABR",
  "SAExploration Holdings Inc.": "SAEX",
  "Sanderson Farms Inc.": "SAFM",
  "Safety Insurance Group Inc.": "SAFT",
  "Sage Therapeutics Inc.": "SAGE",
  "Saia Inc.": "SAIA",
  "Salisbury Bancorp Inc.": "SAL",
  "Salem Media Group Inc.": "SALM",
  "Silvercrest Asset Management Group Inc.": "SAMG",
  "Sanmina Corporation": "SANM",
  "S&W Seed Company": "SANW",
  "Sandy Spring Bancorp Inc.": "SASR",
  "EchoStar Corporation": "SATS",
  "Diversified Restaurant Holdings Inc.": "SAUC",
  "Spirit Airlines Inc.": "SAVE",
  "SBA Communications Corporation": "SBAC",
  "Strongbridge Biopharma plc": "SBBP",
  "Sussex Bancorp": "SBBX",
  "Seacoast Banking Corporation of Florida": "SBCF",
  "Sunshine Bancorp Inc.": "SBCP",
  "SB Financial Group Inc.": "SBFG",
  "SB Financial Group Inc. Depositary Shares each representing a 1/100th interest in a 6.50% Noncumulative convertible perpetual preferred share Series A":
    "SBFGP",
  "Sinclair Broadcast Group Inc.": "SBGI",
  "Star Bulk Carriers Corp.": "SBLK",
  "Star Bulk Carriers Corp. 8.00% Senior Notes Due 2019": "SBLKL",
  "Star Bulk Carriers Corp. 8.30% Senior Notes due 2022": "SBLKZ",
  "Signature Bank": "SBNY",
  "Signature Bank Warrants 12/12/2018": "SBNYW",
  "Stellar Biotechnologies Inc.": "SBOT",
  "Spring Bank Pharmaceuticals Inc.": "SBPH",
  "Sabra Health Care REIT Inc.": "SBRA",
  "Sabra Health Care REIT Inc. 7.125% Preferred Series A": "SBRAP",
  "Southside Bancshares Inc.": "SBSI",
  "Sterling Bancorp Inc.": "SBT",
  "Starbucks Corporation": "SBUX",
  "Saban Capital Acquisition Corp.": "SCAC",
  "Saban Capital Acquisition Corp. Unit": "SCACU",
  "Saban Capital Acquisition Corp. Warrants": "SCACW",
  "Scholastic Corporation": "SCHL",
  "Schnitzer Steel Industries Inc.": "SCHN",
  "Socket Mobile Inc.": "SCKT",
  "Sucampo Pharmaceuticals Inc.": "SCMP",
  "Superconductor Technologies Inc.": "SCON",
  "scPharmaceuticals Inc.": "SCPH",
  "ScanSource Inc.": "SCSC",
  "Shoe Carnival Inc.": "SCVL",
  "SecureWorks Corp.": "SCWX",
  "SCYNEXIS Inc.": "SCYX",
  "First Trust SMID Cap Rising Dividend Achievers ETF": "SDVY",
  "SeaChange International Inc.": "SEAC",
  "Secoo Holding Limited": "SECO",
  "SolarEdge Technologies Inc.": "SEDG",
  "Origin Agritech Limited": "SEED",
  "SEI Investments Company": "SEIC",
  "Sharing Economy International Inc.": "SEII",
  "Selecta Biosciences Inc.": "SELB",
  "Global Self Storage Inc.": "SELF",
  "Seneca Foods Corp. Class A Common Stock": "SENEA",
  "Seneca Foods Corp. Class B Common Stock": "SENEB",
  "Synthesis Energy Systems Inc.": "SES",
  "Sound Financial Bancorp Inc.": "SFBC",
  "ServisFirst Bancshares Inc.": "SFBS",
  "Stitch Fix Inc.": "SFIX",
  "Shutterfly Inc.": "SFLY",
  "Sprouts Farmers Market Inc.": "SFM",
  "Simmons First National Corporation": "SFNC",
  "Southern First Bancshares Inc.": "SFST",
  "SG Blocks Inc.": "SGBX",
  "Superior Uniform Group Inc.": "SGC",
  "Seattle Genetics Inc.": "SGEN",
  "SMART Global Holdings Inc.": "SGH",
  "Sigma Labs Inc.": "SGLB",
  "Sigma Labs Inc. Warrant": "SGLBW",
  "SigmaTron International Inc.": "SGMA",
  "Sangamo Therapeutics Inc.": "SGMO",
  "Scientific Games Corp": "SGMS",
  "SGOCO Group Ltd": "SGOC",
  "Janus Henderson SG Global Quality Income ETF": "SGQI",
  "SPAR Group Inc.": "SGRP",
  "Surgery Partners Inc.": "SGRY",
  "Synergy Pharmaceuticals Inc.": "SGYP",
  "Shore Bancshares Inc": "SHBI",
  "Shenandoah Telecommunications Co": "SHEN",
  "Seanergy Maritime Holdings Corp": "SHIP",
  "Seanergy Maritime Holdings Corp Class A Warrants": "SHIPW",
  "Sears Holdings Corporation": "SHLD",
  "Sears Holdings Corporation Warrant": "SHLDW",
  "A. Schulman Inc.": "SHLM",
  "Shiloh Industries Inc.": "SHLO",
  "Steven Madden Ltd.": "SHOO",
  "Sears Hometown and Outlet Stores Inc.": "SHOS",
  "Shire plc": "SHPG",
  "SharpSpring Inc.": "SHSP",
  "Siebert Financial Corp.": "SIEB",
  "Sientra Inc.": "SIEN",
  "SI Financial Group Inc.": "SIFI",
  "Sify Technologies Limited": "SIFY",
  "Selective Insurance Group Inc.": "SIGI",
  "Sigma Designs Inc.": "SIGM",
  "Silicom Ltd": "SILC",
  "Silicon Motion Technology Corporation": "SIMO",
  "Sina Corporation": "SINA",
  "Sino-Global Shipping America Ltd.": "SINO",
  "Sirius XM Holdings Inc.": "SIRI",
  "SITO Mobile Ltd.": "SITO",
  "SVB Financial Group": "SIVB",
  "SVB Financial Group 7% Cumulative Trust Preferred Securities": "SIVBO",
  "Peak Resorts Inc.": "SKIS",
  "Skyline Medical Inc.": "SKLN",
  "FlexShares Credit-Scored US Corporate Bond Index Fund": "SKOR",
  "Sky Solar Holdings Ltd.": "SKYS",
  "SkyWest Inc.": "SKYW",
  "First Trust Cloud Computing ETF": "SKYY",
  "Silicon Laboratories Inc.": "SLAB",
  "Select Bancorp Inc.": "SLCT",
  "Silgan Holdings Inc.": "SLGN",
  "The Obesity ETF": "SLIM",
  "SLM Corporation": "SLM",
  "SLM Corporation Floating Rate Non-Cumulative Preferred Stock Series B": "SLMBP",
  "Soleno Therapeutics Inc.": "SLNO",
  "Soleno Therapeutics Inc. Series A Warrant": "SLNOW",
  "Simulations Plus Inc.": "SLP",
  "Solar Capital Ltd.": "SLRC",
  "SELLAS Life Sciences Group Inc.": "SLS",
  "Credit Suisse Silver Shares Covered Call Exchange Traded Notes": "SLVO",
  "Southern Missouri Bancorp Inc.": "SMBC",
  "SmartFinancial Inc.": "SMBK",
  "Super Micro Computer Inc.": "SMCI",
  "AlphaMark Actively Managed Small Cap ETF": "SMCP",
  "Sharps Compliance Corp.": "SMED",
  "Schmitt Industries Inc.": "SMIT",
  "Summit Financial Group Inc.": "SMMF",
  "Summit Therapeutics plc": "SMMT",
  "The Simply Good Foods Company": "SMPL",
  "The Simply Good Foods Company Warrant": "SMPLW",
  "Stein Mart Inc.": "SMRT",
  "Smith Micro Software Inc.": "SMSI",
  "Semtech Corporation": "SMTC",
  "SMTC Corporation": "SMTX",
  "Inventure Foods Inc.": "SNAK",
  "Sun Bancorp Inc.": "SNBC",
  "Sleep Number Corporation": "SNBR",
  "Synchronoss Technologies Inc.": "SNCR",
  "Smart Sand Inc.": "SND",
  "Sundance Energy Australia Limited": "SNDE",
  "Syndax Pharmaceuticals Inc.": "SNDX",
  "SenesTech Inc.": "SNES",
  "Security National Financial Corporation Class A Common Stock": "SNFCA",
  "Soligenix Inc.": "SNGX",
  "Soligenix Inc. Warrant": "SNGXW",
  "Sun Hydraulics Corporation": "SNHY",
  "Senomyx Inc.": "SNMX",
  "Sienna Biopharmaceuticals Inc.": "SNNA",
  "Sonoma Pharmaceuticals Inc.": "SNOA",
  "Sonoma Pharmaceuticals Inc. Warrants": "SNOAW",
  "Synopsys Inc.": "SNPS",
  "Global X Internet of Things ETF": "SNSR",
  "Sunesis Pharmaceuticals Inc.": "SNSS",
  "Global X Social Media ETF": "SOCL",
  "SodaStream International Ltd.": "SODA",
  "Sonic Foundry Inc.": "SOFO",
  "Sotherly Hotels Inc.": "SOHO",
  "Sotherly Hotels Inc. 8.0% Series B Cumulative Redeemable Perpetual Preferred Stock": "SOHOB",
  "Sotherly Hotels Inc. 7.875% Series C Cumulative Redeemable Perpetual Preferred Stock": "SOHOO",
  "Sohu.com Inc.": "SOHU",
  "Southern National Bancorp of Virginia Inc.": "SONA",
  "Sonic Corp.": "SONC",
  "SORL Auto Parts Inc.": "SORL",
  "iShares PHLX SOX Semiconductor Sector Index Fund": "SOXX",
  "SP Plus Corporation": "SP",
  "Spartan Motors Inc.": "SPAR",
  "SuperCom Ltd.": "SPCB",
  "Spherix Incorporated": "SPEX",
  "Sophiris Bio Inc.": "SPHS",
  "SPI Energy Co. Ltd.": "SPI",
  "Siliconware Precision Industries Company Ltd.": "SPIL",
  "Spark Energy Inc.": "SPKE",
  "Spark Energy Inc. 8.75% Series A Fixed-to-Floating Rate Cumulative Redeemable Perpetual Preferred Stock": "SPKEP",
  "Splunk Inc.": "SPLK",
  "SeaSpine Holdings Corporation": "SPNE",
  "Sapiens International Corporation N.V.": "SPNS",
  "Spok Holdings Inc.": "SPOK",
  "Spectrum Pharmaceuticals Inc.": "SPPI",
  "Spero Therapeutics Inc.": "SPRO",
  "Support.com Inc.": "SPRT",
  "SPS Commerce Inc.": "SPSC",
  "SpartanNash Company": "SPTN",
  "Sportsman's Warehouse Holdings Inc.": "SPWH",
  "SunPower Corporation": "SPWR",
  "Sequential Brands Group Inc.": "SQBG",
  "Legg Mason Small-Cap Quality Value ETF": "SQLV",
  "ProShares UltraPro Short QQQ": "SQQQ",
  "Active Alts Contrarian ETF": "SQZZ",
  "Social Reality Inc.": "SRAX",
  "1st Source Corporation": "SRCE",
  "Stericycle Inc.": "SRCL",
  "Stericycle Inc. Depository Receipt": "SRCLP",
  "Surmodics Inc.": "SRDX",
  "Global X SuperDividend REIT ETF": "SRET",
  "ServiceSource International Inc.": "SREV",
  "Sorrento Therapeutics Inc.": "SRNE",
  "Sarepta Therapeutics Inc.": "SRPT",
  "Sierra Oncology Inc.": "SRRA",
  "Sensus Healthcare Inc.": "SRTS",
  "Sensus Healthcare Inc. Warrant": "SRTSW",
  "Silver Run Acquisition Corporation II": "SRUN",
  "Silver Run Acquisition Corporation II Unit": "SRUNU",
  "Silver Run Acquisition Corporation II Warrant": "SRUNW",
  "South State Corporation": "SSB",
  "Summit State Bank": "SSBI",
  "Seven Stars Cloud Group Inc.": "SSC",
  "Stewardship Financial Corp": "SSFN",
  "Strata Skin Sciences Inc.": "SSKN",
  "SS&C Technologies Holdings Inc.": "SSNC",
  "SilverSun Technologies Inc.": "SSNT",
  "SSR Mining Inc.": "SSRM",
  "ShotSpotter Inc.": "SSTI",
  "Stratasys Ltd.": "SSYS",
  "STAAR Surgical Company": "STAA",
  "Staffing 360 Solutions Inc.": "STAF",
  "Student Transportation Inc": "STB",
  "S&T Bancorp Inc.": "STBA",
  "State Bank Financial Corporation.": "STBZ",
  "SteadyMed Ltd.": "STDY",
  "State Auto Financial Corporation": "STFC",
  "SunOpta Inc.": "STKL",
  "The ONE Group Hospitality Inc.": "STKS",
  "Steel Dynamics Inc.": "STLD",
  "Stellar Acquisition III Inc.": "STLR",
  "Stellar Acquisition III Inc. Unit": "STLRU",
  "Stellar Acquisition III Inc. Warrants": "STLRW",
  "Stanley Furniture Company Inc.": "STLY",
  "Stemline Therapeutics Inc.": "STML",
  "Stamps.com Inc.": "STMP",
  "Sentinel Energy Services Inc.": "STNL",
  "Sentinel Energy Services Inc. Unit": "STNLU",
  "Sentinel Energy Services Inc. Warrant": "STNLW",
  "Strayer Education Inc.": "STRA",
  "Sterling Construction Company Inc": "STRL",
  "Streamline Health Solutions Inc.": "STRM",
  "Stratus Properties Inc.": "STRS",
  "Strattec Security Corporation": "STRT",
  "Seagate Technology PLC": "STX",
  "Summer Infant Inc.": "SUMR",
  "Solar Senior Capital Ltd.": "SUNS",
  "Sunworks Inc.": "SUNW",
  "Supernus Pharmaceuticals Inc.": "SUPN",
  "iShares ESG 1-5 Year USD Corporate Bond ETF": "SUSB",
  "iShares ESG USD Corporate Bond ETF": "SUSC",
  "Sinovac Biotech Ltd.": "SVA",
  "Severn Bancorp Inc": "SVBI",
  "Savara Inc.": "SVRA",
  "Firsthand Technology Value Fund Inc.": "SVVC",
  "ALPS/Dorsey Wright Sector Momentum ETF": "SWIN",
  "Sierra Wireless Inc.": "SWIR",
  "Skyworks Solutions Inc.": "SWKS",
  "Stock Yards Bancorp Inc.": "SYBT",
  "Synlogic Inc.": "SYBX",
  "Sykes Enterprises Incorporated": "SYKE",
  "Symantec Corporation": "SYMC",
  "Synaptics Incorporated": "SYNA",
  "Synacor Inc.": "SYNC",
  "Syneos Health Inc.": "SYNH",
  "Synalloy Corporation": "SYNL",
  "Syntel Inc.": "SYNT",
  "Sypris Solutions Inc.": "SYPR",
  "Syros Pharmaceuticals Inc.": "SYRS",
  "Del Taco Restaurants Inc.": "TACO",
  "Del Taco Restaurants Inc. Warrants": "TACOW",
  "TransAct Technologies Incorporated": "TACT",
  "Taitron Components Incorporated": "TAIT",
  "Tantech Holdings Ltd.": "TANH",
  "Barclays Inverse US Treasury Composite ETN": "TAPR",
  "Carrols Restaurant Group Inc.": "TAST",
  "TAT Technologies Ltd.": "TATT",
  "Liberty Tax Inc.": "TAX",
  "Taylor Devices Inc.": "TAYD",
  "The Bancorp Inc.": "TBBK",
  "Triumph Bancorp Inc.": "TBK",
  "Territorial Bancorp Inc.": "TBNK",
  "Theravance Biopharma Inc.": "TBPH",
  "Texas Capital Bancshares Inc.": "TCBI",
  "Texas Capital Bancshares Inc. 6.50% Subordinated Notes due 2042": "TCBIL",
  "Texas Capital Bancshares Inc. Non Cumulative Preferred Perpetual Stock Series A": "TCBIP",
  "Texas Capital Bancshares Inc. Warrants 01/16/2019": "TCBIW",
  "TriCo Bancshares": "TCBK",
  "Technical Communications Corporation": "TCCO",
  "The Community Financial Corporation": "TCFC",
  "Tactile Systems Technology Inc.": "TCMD",
  "TRACON Pharmaceuticals Inc.": "TCON",
  "TCP Capital Corp.": "TCPC",
  "THL Credit Inc.": "TCRD",
  "Tucows Inc.": "TCX",
  "First Trust NASDAQ Technology Dividend Index Fund": "TDIV",
  "Atlassian Corporation Plc": "TEAM",
  "Tech Data Corporation": "TECD",
  "Bio-Techne Corp": "TECH",
  "Tarena International Inc.": "TEDU",
  "Tellurian Inc.": "TELL",
  "Tenax Therapeutics Inc.": "TENX",
  "TerraForm Power Inc.": "TERP",
  "Tesco Corporation": "TESO",
  "TESSCO Technologies Incorporated": "TESS",
  "TFS Financial Corporation": "TFSL",
  "TransGlobe Energy Corporation": "TGA",
  "Tecogen Inc.": "TGEN",
  "Tecnoglass Inc.": "TGLS",
  "TG Therapeutics Inc.": "TGTX",
  "First Financial Corporation Indiana": "THFF",
  "Gentherm Inc": "THRM",
  "Truett-Hurst Inc.": "THST",
  "TICC Capital Corp.": "TICC",
  "TICC Capital Corp. 6.50% Notes due 2024": "TICCL",
  TiGenix: "TIG",
  "Till Capital Ltd.": "TIL",
  "Interface Inc.": "TILE",
  "Tiptree Inc.": "TIPT",
  "Top Image Systems Ltd.": "TISA",
  "Titan Machinery Inc.": "TITN",
  "TiVo Corporation": "TIVO",
  "Tandy Leather Factory Inc.": "TLF",
  "Teligent Inc.": "TLGT",
  "Talend S.A.": "TLND",
  "T-Mobile US Inc. 5.50% Mandatory Convertible Preferred Stock Series A": "TMUSP",
  "Telenav Inc.": "TNAV",
  "Tandem Diabetes Care Inc.": "TNDM",
  "Tintri Inc.": "TNTR",
  "Tonix Pharmaceuticals Holding Corp.": "TNXP",
  "Tocagen Inc.": "TOCA",
  "TOP Ships Inc.": "TOPS",
  "TOR Minerals International Inc": "TORM",
  "Tuniu Corporation": "TOUR",
  "Towne Bank": "TOWN",
  "TPI Composites Inc.": "TPIC",
  "TapImmune Inc.": "TPIV",
  "ProShares UltraPro QQQ": "TQQQ",
  "Two River Bancorp": "TRCB",
  "Torchlight Energy Resources Inc.": "TRCH",
  "LendingTree Inc.": "TREE",
  "Tabula Rasa HealthCare Inc.": "TRHC",
  "Trinity Biotech plc": "TRIB",
  "Trillium Therapeutics Inc.": "TRIL",
  "TripAdvisor Inc.": "TRIP",
  "Trimble Inc.": "TRMB",
  "Trustmark Corporation": "TRMK",
  "Tremont Mortgage Trust": "TRMT",
  "Transcat Inc.": "TRNS",
  "TrovaGene Inc.": "TROV",
  "T. Rowe Price Group Inc.": "TROW",
  "Therapix Biosciences Ltd.": "TRPX",
  "TriMas Corporation": "TRS",
  "TrustCo Bank Corp NY": "TRST",
  "TrueCar Inc.": "TRUE",
  "trivago N.V.": "TRVG",
  "Trevena Inc.": "TRVN",
  "Timberland Bancorp Inc.": "TSBK",
  "TriState Capital Holdings Inc.": "TSC",
  "Tractor Supply Company": "TSCO",
  "Tower Semiconductor Ltd.": "TSEM",
  "The Stars Group Inc.": "TSG",
  "Tesla Inc.": "TSLA",
  "TSR Inc.": "TSRI",
  "TESARO Inc.": "TSRO",
  "TheStreet Inc.": "TST",
  "The Trade Desk Inc.": "TTD",
  "TTEC Holdings Inc.": "TTEC",
  "Tetra Tech Inc.": "TTEK",
  "TechTarget Inc.": "TTGT",
  "TTM Technologies Inc.": "TTMI",
  "Titan Pharmaceuticals Inc.": "TTNP",
  "T2 Biosystems Inc.": "TTOO",
  "Tetraphase Pharmaceuticals Inc.": "TTPH",
  "Tile Shop Hldgs Inc.": "TTS",
  "Take-Two Interactive Software Inc.": "TTWO",
  "Tuesday Morning Corp.": "TUES",
  "180 Degree Capital Corp.": "TURN",
  "Mammoth Energy Services Inc.": "TUSK",
  "VelocityShares Daily 2x VIX Short Term ETN": "TVIX",
  "VelocityShares Daily 2x VIX Medium Term ETN": "TVIZ",
  "Tivity Health Inc.": "TVTY",
  "Twin Disc Incorporated": "TWIN",
  "Trans World Entertainment Corp.": "TWMC",
  "Hostess Brands Inc.": "TWNK",
  "Hostess Brands Inc. Warrants": "TWNKW",
  "2U Inc.": "TWOU",
  "Texas Instruments Incorporated": "TXN",
  "Texas Roadhouse Inc.": "TXRH",
  "Shineco Inc.": "TYHT",
  "Tyme Technologies Inc.": "TYME",
  "Monotype Imaging Holdings Inc.": "TYPE",
  Travelzoo: "TZOO",
  "iShares MSCI UAE ETF": "UAE",
  "United Bancorp Inc.": "UBCP",
  "United Security Bancshares": "UBFO",
  "Proshares UltraPro Nasdaq Biotechnology": "UBIO",
  "United Financial Bancorp Inc.": "UBNK",
  "Ubiquiti Networks Inc.": "UBNT",
  "United Bancshares Inc.": "UBOH",
  "Union Bankshares Corporation": "UBSH",
  "United Bankshares Inc.": "UBSI",
  "United Community Bancorp": "UCBA",
  "United Community Banks Inc.": "UCBI",
  "United Community Financial Corp.": "UCFC",
  "Ultra Clean Holdings Inc.": "UCTT",
  "Legg Mason US Diversified Core ETF": "UDBI",
  "Universal Electronics Inc.": "UEIC",
  "Net 1 UEPS Technologies Inc.": "UEPS",
  "United Fire Group Inc": "UFCS",
  "Universal Forest Products Inc.": "UFPI",
  "UFP Technologies Inc.": "UFPT",
  "United-Guardian Inc.": "UG",
  "VelocityShares 3x Long Gold ETN": "UGLD",
  Amerco: "UHAL",
  "United Insurance Holdings Corp.": "UIHC",
  "Ultralife Corporation": "ULBI",
  "Universal Logistics Holdings Inc.": "ULH",
  "Ulta Beauty Inc.": "ULTA",
  "The Ultimate Software Group Inc.": "ULTI",
  "UMB Financial Corporation": "UMBF",
  "Umpqua Holdings Corporation": "UMPQ",
  "Unico American Corporation": "UNAM",
  "Union Bankshares Inc.": "UNB",
  "United Natural Foods Inc.": "UNFI",
  "Uniti Group Inc.": "UNIT",
  "Unity Bancorp Inc.": "UNTY",
  "Urban One Inc.": "UONE",
  "Upland Software Inc.": "UPLD",
  "Urban Outfitters Inc.": "URBN",
  "UroGen Pharma Ltd.": "URGN",
  "USA Truck Inc.": "USAK",
  "Universal Stainless & Alloy Products Inc.": "USAP",
  "USA Technologies Inc.": "USAT",
  "USA Technologies Inc. Preferred Stock": "USATP",
  "U.S. Gold Corp.": "USAU",
  "U S Concrete Inc.": "USCR",
  "U.S. Energy Corp.": "USEG",
  "PowerShares Russell 1000 Low Beta Equal Weight Portfolio": "USLB",
  "United States Lime & Minerals Inc.": "USLM",
  "VelocityShares 3x Long Silver ETN": "USLV",
  "Principal U.S. Mega-Cap Multi-Factor Index ETF": "USMC",
  "Credit Suisse X-Links Crude Oil Shares Covered Call ETN": "USOI",
  "ETFIS Series Trust I Reaves Utilities": "UTES",
  "United Therapeutics Corporation": "UTHR",
  "Utah Medical Products Inc.": "UTMD",
  "UTStarcom Holdings Corp": "UTSI",
  "Univest Corporation of Pennsylvania": "UVSP",
  "Value Line Inc.": "VALU",
  "Validea Market Legends ETF": "VALX",
  "Village Bank and Trust Financial Corp.": "VBFC",
  "VBI Vaccines Inc.": "VBIV",
  "Vascular Biogenics Ltd.": "VBLT",
  "Vident Core U.S. Bond Strategy Fund": "VBND",
  "Veritex Holdings Inc.": "VBTX",
  "Vericel Corporation": "VCEL",
  "Vanguard Intermediate-Term Corporate Bond ETF": "VCIT",
  "Vanguard Long-Term Corporate Bond ETF": "VCLT",
  "Vanguard Short-Term Corporate Bond ETF": "VCSH",
  "Veracyte Inc.": "VCYT",
  "VASCO Data Security International Inc.": "VDSI",
  "Videocon d2h Limited": "VDTH",
  "Vantage Energy Acquisition Corp.": "VEAC",
  "Vantage Energy Acquisition Corp. Unit": "VEACU",
  "Vantage Energy Acquisition Corp. Warrant": "VEACW",
  "Veeco Instruments Inc.": "VECO",
  "VEON Ltd.": "VEON",
  "Veritone Inc.": "VERI",
  "Veru Inc.": "VERU",
  "Vanguard Intermediate-Term Treasury ETF": "VGIT",
  "Vanguard Long-Treasury ETF": "VGLT",
  "Vanguard Short-Term Treasury ETF": "VGSH",
  "Viacom Inc.": "VIA",
  "Viavi Solutions Inc.": "VIAV",
  "Vical Incorporated": "VICL",
  "Vicor Corporation": "VICR",
  "Vident International Equity Fund": "VIDI",
  "Vanguard International Dividend Appreciation ETF": "VIGI",
  "VelocityShares VIX Short Term ETN": "VIIX",
  "VelocityShares VIX Medium Term ETN": "VIIZ",
  "Virco Manufacturing Corporation": "VIRC",
  "Virtu Financial Inc.": "VIRT",
  "Viveve Medical Inc.": "VIVE",
  "Meridian Bioscience Inc.": "VIVO",
  "Viking Therapeutics Inc.": "VKTX",
  "Viking Therapeutics Inc. Warrants": "VKTXW",
  "Village Super Market Inc. Class A Common Stock": "VLGEA",
  "Valeritas Holdings Inc.": "VLRX",
  "Vanguard Mortgage-Backed Securities ETF": "VMBS",
  "Vanda Pharmaceuticals Inc.": "VNDA",
  "21Vianet Group Inc.": "VNET",
  "Viper Energy Partners LP": "VNOM",
  "Vanguard Global ex-U.S. Real Estate ETF": "VNQI",
  "Vodafone Group Plc": "VOD",
  "Vanguard Russell 1000 ETF": "VONE",
  "Vanguard Russell 1000 Growth ETF": "VONG",
  "Vanguard Russell 1000 Value ETF": "VONV",
  "VOXX International Corporation": "VOXX",
  "Vera Bradley Inc.": "VRA",
  "ViewRay Inc.": "VRAY",
  "Varex Imaging Corporation": "VREX",
  "PowerShares Variable Rate Investment Grade Portfolio": "VRIG",
  "Vermillion Inc.": "VRML",
  "Verona Pharma plc": "VRNA",
  "Varonis Systems Inc.": "VRNS",
  "Verint Systems Inc.": "VRNT",
  "Verisk Analytics Inc.": "VRSK",
  "VeriSign Inc.": "VRSN",
  "Virtus Investment Partners Inc.": "VRTS",
  "Virtus Investment Partners Inc. 7.25% Series D Mandatory Convertible Preferred Stock": "VRTSP",
  "Virtusa Corporation": "VRTU",
  "Vertex Pharmaceuticals Incorporated": "VRTX",
  "Versartis Inc.": "VSAR",
  "ViaSat Inc.": "VSAT",
  "VictoryShares Dividend Accelerator ETF": "VSDA",
  "VSE Corporation": "VSEC",
  "VictoryShares US Multi-Factor Minimum Volatility ETF": "VSMV",
  "Verastem Inc.": "VSTM",
  "Vanguard Total Corporate Bond ETF": "VTC",
  "VistaGen Therapeutics Inc.": "VTGN",
  "Vanguard Russell 3000 ETF": "VTHR",
  "Vanguard Short-Term Inflation-Protected Securities Index Fund": "VTIP",
  "Vital Therapies Inc.": "VTL",
  "Vertex Energy Inc": "VTNR",
  "vTv Therapeutics Inc.": "VTVT",
  "Vanguard Russell 2000 Growth ETF": "VTWG",
  "Vanguard Russell 2000 ETF": "VTWO",
  "Vanguard Russell 2000 Value ETF": "VTWV",
  "Vident Core US Equity ETF": "VUSE",
  "Vuzix Corporation": "VUZI",
  "VivoPower International PLC": "VVPR",
  "VIVUS Inc.": "VVUS",
  "Vanguard Emerging Markets Government Bond ETF": "VWOB",
  "Vanguard Total International Stock ETF": "VXUS",
  "Voyager Therapeutics Inc.": "VYGR",
  "Vanguard International High Dividend Yield ETF": "VYMI",
  "Westamerica Bancorporation": "WABC",
  "Washington Federal Inc.": "WAFD",
  "Washington Federal Inc. Warrants 11/14/2018": "WAFDW",
  "Washington Trust Bancorp Inc.": "WASH",
  "Energous Corporation": "WATT",
  "Weibo Corporation": "WB",
  "WCF Bancorp Inc.": "WCFB",
  "Western Digital Corporation": "WDC",
  "WD-40 Company": "WDFC",
  "Web.com Group Inc.": "WEB",
  "Wellesley Bancorp Inc.": "WEBK",
  "Wendy's Company (The)": "WEN",
  "Werner Enterprises Inc.": "WERN",
  "WisdomTree Investments Inc.": "WETF",
  "Weyco Group Inc.": "WEYS",
  "WashingtonFirst Bankshares Inc": "WFBI",
  "WhiteHorse Finance Inc.": "WHF",
  "WhiteHorse Finance Inc. 6.50% Senior Notes due 2020": "WHFBL",
  "Wilhelmina International Inc.": "WHLM",
  "Wheeler Real Estate Investment Trust Inc.": "WHLR",
  "Wheeler Real Estate Investment Trust Inc. Series D Cumulative Preferred Stock": "WHLRD",
  "Wheeler Real Estate Investment Trust Inc. Preferred Stock": "WHLRP",
  "Wheeler Real Estate Investment Trust Inc. Warrants": "WHLRW",
  "Boingo Wireless Inc.": "WIFI",
  "G. Willi-Food International  Ltd.": "WILC",
  "Windstream Holdings Inc.": "WIN",
  "Winmark Corporation": "WINA",
  "Wingstop Inc.": "WING",
  "Wins Finance Holdings Inc.": "WINS",
  "Encore Wire Corporation": "WIRE",
  "Wix.com Ltd.": "WIX",
  "Workhorse Group Inc.": "WKHS",
  "Westmoreland Coal Company": "WLB",
  "Willdan Group Inc.": "WLDN",
  "Willis Lease Finance Corporation": "WLFC",
  "Willis Towers Watson Public Limited Company": "WLTW",
  "Wright Medical Group N.V.": "WMGI",
  "Wright Medical Group N.V. Contingent Value Right": "WMGIZ",
  "WMIH Corp.": "WMIH",
  "Western New England Bancorp Inc.": "WNEB",
  "iShares S&P Global Timber & Forestry Index Fund": "WOOD",
  "WPCS International Incorporated": "WPCS",
  "WPP plc American Depositary Shares": "WPP",
  "Westport Fuel Systems Inc": "WPRT",
  "World Acceptance Corporation": "WRLD",
  "Pensare Acquisition Corp.": "WRLS",
  "Pensare Acquisition Corp. Right": "WRLSR",
  "Pensare Acquisition Corp. Unit": "WRLSU",
  "Pensare Acquisition Corp. Warrant": "WRLSW",
  "WesBanco Inc.": "WSBC",
  "Waterstone Financial Inc.": "WSBF",
  "WillScot Corporation": "WSC",
  "WSI Industries Inc.": "WSCI",
  "WillScot Corporation Warrant": "WSCWW",
  "WSFS Financial Corporation": "WSFS",
  "Wayside Technology Group Inc.": "WSTG",
  "Westell Technologies Inc.": "WSTL",
  "West Bancorporation": "WTBA",
  "Wintrust Financial Corporation": "WTFC",
  "Wintrust Financial Corporation Fixed-to-Floating Rate Non-Cumulative Perpetual Preferred Stock Series D": "WTFCM",
  "Wintrust Financial Corporation Warrants": "WTFCW",
  "WAVE Life Sciences Ltd.": "WVE",
  "WVS Financial Corp.": "WVFC",
  "Willamette Valley Vineyards Inc.": "WVVI",
  "Willamette Valley Vineyards Inc. Series A Redeemable Preferred Stock": "WVVIP",
  "Woodward Inc.": "WWD",
  "Westwater Resources Inc.": "WWR",
  "JM Global Holding Company": "WYIG",
  "JM Global Holding Company Unit": "WYIGU",
  "JM Global Holding Company Warrants": "WYIGW",
  "Wynn Resorts Limited": "WYNN",
  "Xenetic Biosciences Inc.": "XBIO",
  "XBiotech Inc.": "XBIT",
  "Xenith Bankshares Inc.": "XBKS",
  "Xcerra Corporation": "XCRA",
  "Exela Technologies Inc.": "XELA",
  "Xcel Brands Inc": "XELB",
  "Xenon Pharmaceuticals Inc.": "XENE",
  "Intersect ENT Inc.": "XENT",
  "XG Technology Inc": "XGTI",
  "XG Technology Inc Warrants (1 Wt & 824.40 to purchase 1 shr).": "XGTIW",
  "VelocityShares Daily Inverse VIX Short Term ETN": "XIV",
  "Xilinx Inc.": "XLNX",
  "Acceleron Pharma Inc.": "XLRN",
  "Xencor Inc.": "XNCR",
  "Xunlei Limited": "XNET",
  "Extraction Oil & Gas Inc.": "XOG",
  "XOMA Corporation": "XOMA",
  "The ExOne Company": "XONE",
  "Xperi Corporation": "XPER",
  "Xplore Technologies Corp": "XPLR",
  "DENTSPLY SIRONA Inc.": "XRAY",
  "XpresSpa Group Inc.": "XSPA",
  "XTL Biopharmaceuticals Ltd.": "XTLB",
  "First Trust International Multi-Asset Diversified Income Index Fund": "YDIV",
  "Yulong Eco-Materials Limited": "YECO",
  "Yangtze River Development Limited": "YERR",
  "Youngevity International Inc.": "YGYI",
  "Yintech Investment Holdings Limited": "YIN",
  "Global X Yieldco Index ETF": "YLCO",
  "ClearBridge Dividend Strategy ESG ETF": "YLDE",
  "Yandex N.V.": "YNDX",
  "YogaWorks Inc.": "YOGA",
  "The York Water Company": "YORW",
  "YRC Worldwide Inc.": "YRCW",
  "Yield10 Bioscience Inc.": "YTEN",
  "Yatra Online Inc.": "YTRA",
  "YY Inc.": "YY",
  "ZAGG Inc": "ZAGG",
  "ZAIS Group Holdings Inc.": "ZAIS",
  ZAZZT: "ZAZZT",
  "ProShares UltraPro Short NASDAQ Biotechnology": "ZBIO",
  "Zebra Technologies Corporation": "ZBRA",
  ZBZZT: "ZBZZT",
  "Super Montage IPO X Tst Security SM IPO X 3": "ZCZZT",
  "Zealand Pharma A/S": "ZEAL",
  "Olympic Steel Inc.": "ZEUS",
  "Zafgen Inc.": "ZFGN",
  "Zillow Group Inc.": "ZG",
  "Zogenix Inc.": "ZGNX",
  "Zions Bancorporation": "ZION",
  "Zions Bancorporation Warrants 05/21/2020": "ZIONW",
  "Zions Bancorporation Warrants": "ZIONZ",
  "ZIOPHARM Oncology Inc": "ZIOP",
  "VelocityShares Daily Inverse VIX Medium Term ETN": "ZIV",
  "Zix Corporation": "ZIXI",
  "NASDAQ TEST STOCK": "ZJZZT",
  "ZK International Group Co. Ltd": "ZKIN",
  "Zai Lab Limited": "ZLAB",
  "Zion Oil & Gas Inc": "ZN",
  "Zynga Inc.": "ZNGA",
  "Zosano Pharma Corporation": "ZSAN",
  "Zumiez Inc.": "ZUMZ",
  "NextShares ETMF Test": "ZVZZC",
  "Zynerba Pharmaceuticals Inc.": "ZYNE"
};
// =========================================================================================
//                                      EXPORTS
// =========================================================================================
exports.handler = stocobsHandler;
